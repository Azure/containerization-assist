...
function main() {
  ...
  if (!binaryPath) {
    ...
    logInfo('     npm uninstall @containerization-assist/mcp-server');
    logInfo('     npm install @containerization-assist/mcp-server');
    logInfo('\n  2. Build from source:');
  ...
  child.on('error', (err) => {
    if (err.code === 'ENOENT') {
      ...
      logInfo('  npm uninstall -g @containerization-assist/mcp-server');
      logInfo('  npm install -g @containerization-assist/mcp-server');
