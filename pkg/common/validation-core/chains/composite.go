...
func (c *CompositeValidator) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.NonGenericResult {
	startTime := time.Now()
...
// AddValidator adds a validator to the chain (interface method)
func (c *CompositeValidator) AddValidator(validator core.Validator) core.ValidatorChain {
	return c.Add(validator)
}

// RemoveValidator removes a validator from the chain by name
func (c *CompositeValidator) RemoveValidator(name string) bool {
	for i, validator := range c.validators {
		if validator.GetName() == name {
			// Remove the validator at index i
...
func (w *conditionalValidatorWrapper) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.NonGenericResult {
...
func (p *ParallelValidator) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.NonGenericResult {
	startTime := time.Now()
	...
	resultChan := make(chan *core.NonGenericResult, len(p.validators))
