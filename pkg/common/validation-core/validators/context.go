...
func (c *ContextValidator) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.NonGenericResult {
...
func (c *ContextValidator) validateDockerfileContext(data interface{}, result *core.NonGenericResult, options *core.ValidationOptions) {
...
func (c *ContextValidator) validateContextInstruction(instruction ContextInstruction, contextPath string, result *core.NonGenericResult) {
	...
	if filepath.IsAbs(instruction.Source) {
		...
		warning.Error.WithLine(instruction.Line)
...
func (c *ContextValidator) validateBuildFiles(data interface{}, result *core.NonGenericResult, options *core.ValidationOptions) {
	...
	for _, file := range files {
		...
		if c.isSensitiveFile(file) {
			...
			warning.Error.Suggestions = append(warning.Error.Suggestions, "Add sensitive files to .dockerignore")
			result.AddWarning(warning)
		}
	}
...
func (c *ContextValidator) validateCopyOperations(data interface{}, result *core.NonGenericResult, options *core.ValidationOptions) {
...
func (c *ContextValidator) autoDetectAndValidate(data interface{}, result *core.NonGenericResult, options *core.ValidationOptions) {
	switch v := data.(type) {
	case string:
...
func (c *ContextValidator) checkSensitiveFiles(path string, line int, result *core.NonGenericResult) {
	...
	for _, pattern := range sensitivePatterns {
		if matched, _ := filepath.Match(pattern, filepath.Base(lowercasePath)); matched {
			...
			warning.Error.WithLine(line)
			warning.Error.Suggestions = append(warning.Error.Suggestions, "Ensure this file should be included in the image")
			result.AddWarning(warning)
			break
		}
	}
...
func (c *ContextValidator) validateAddInstruction(instruction ContextInstruction, result *core.NonGenericResult) {
...
func (p *PathValidator) ValidatePath(path string, result *core.NonGenericResult) {
	if path == "" {
		result.AddError(core.NewError(
			"EMPTY_PATH",
