...
func (f *FormatValidator) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.NonGenericResult {
...
func (f *FormatValidator) validateEmail(data interface{}, result *core.NonGenericResult) {
	...
	if _, err := mail.ParseAddress(emailStr); err != nil {
		...
		if !f.emailRegex.MatchString(emailStr) {
			...
			err.Suggestions = append(err.Suggestions, "Email should be in format: user@domain.com")
			result.AddError(err)
		}
	}
}
...
func (f *FormatValidator) validateURL(data interface{}, result *core.NonGenericResult) {
...
func (f *FormatValidator) validateJSON(data interface{}, result *core.NonGenericResult) {
	var jsonStr string
	switch v := data.(type) {
	case string:
		jsonStr = v
	...
	if err := json.Unmarshal([]byte(jsonStr), &jsonData); err != nil {
		...
		if syntaxErr, ok := err.(*json.SyntaxError); ok {
			...
			err.Context["offset"] = syntaxErr.Offset
			result.AddError(err)
		} else {
			result.AddError(core.NewError(
				"INVALID_JSON_FORMAT",
...
func (f *FormatValidator) validateYAML(data interface{}, result *core.NonGenericResult) {
	var yamlStr string
	switch v := data.(type) {
	case string:
		yamlStr = v
...
func (f *FormatValidator) autoDetectAndValidate(data interface{}, result *core.NonGenericResult) {
	str, ok := data.(string)
	if !ok {
