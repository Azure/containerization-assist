...
func CheckPodStatus(ctx context.Context, kube KubeRunner, namespace string, labelSelector string, timeout time.Duration) (bool, string) {
	endTime := time.Now().Add(timeout)

	for time.Now().Before(endTime) {
		readableOutputStr, err := kube.GetPods(ctx, namespace, labelSelector)
		...
		outputStr, err := kube.GetPodsJSON(ctx, namespace, labelSelector)
		if err != nil {
...
func DeployAndVerifySingleManifest(ctx context.Context, kube KubeRunner, manifestPath string, isDeployment bool) (bool, string, error) {
	...
	outputStr, err := kube.Apply(ctx, manifestPath)
	...
	podSuccess, podOutput := CheckPodStatus(ctx, kube, namespace, labelSelector, time.Minute)
	if !podSuccess {
		logger.Debugf("    Retrieving logs for pods with label selector %s in namespace %s", labelSelector, namespace)
		podLogs, err := GetDeploymentLogs(ctx, kube, labelSelector, namespace)
		if err != nil {
		...
		deleteOutput, err := kube.DeleteDeployment(ctx, manifestPath)
		if err != nil {
...
func GetDeploymentLogs(ctx context.Context, kube KubeRunner, labelSelector string, namespace string) (string, error) {
