...
func (rm *RegistryManager) categorizePullError(err error, output string) string {
	...
	// Special case: "repository does not exist" should be not_found even if it has "denied"
	if strings.Contains(errStr, "repository does not exist") || strings.Contains(outputStr, "repository does not exist") {
		return "not_found"
	}
