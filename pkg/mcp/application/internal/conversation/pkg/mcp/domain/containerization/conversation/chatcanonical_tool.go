...
func NewchatcanonicalTool(sessionManager session.UnifiedSessionManager, logger zerolog.Logger) api.Tool {
...
func (t *chatcanonicalTool) Name() string {
	return "chatcanonical"
}

// Description implements api.Tool
func (t *chatcanonicalTool) Description() string {
	return ""
}

// Schema implements api.Tool
func (t *chatcanonicalTool) Schema() api.ToolSchema {
...
func (t *chatcanonicalTool) Execute(ctx context.Context, input api.ToolInput) (api.ToolOutput, error) {
	// Validate session ID
	if input.SessionID == "" {
