...
func (ps *PromptServiceImpl) shouldAutoRunPreFlightChecks(state *ConversationState, _ string) bool {
	...
	contextEmpty := len(state.Context) == 0
...
func (ps *PromptServiceImpl) handlePreFlightChecks(_ context.Context, state *ConversationState, input string) *ConversationResponse {
	...
	if true {
		...
		if err := ps.sessionManager.UpdateSession(context.Background(), state.SessionState.SessionID, func(sess *session.SessionState) error {
