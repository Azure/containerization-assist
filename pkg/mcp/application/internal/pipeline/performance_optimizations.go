...
func (po *PerformanceOptimizer) OptimizeDockerOperation(ctx context.Context, operationType, sessionID string, args map[string]interface{}) (interface{}, error) {
	...
	case "pull":
		imageRef := po.safeStringFromMap(args, "image_ref")
		if imageRef == "" {
			return nil, errors.NewError().Messagef("image_ref is required for pull operation").WithLocation().Build()
		}
	...
	case "push":
		imageRef := po.safeStringFromMap(args, "image_ref")
		if imageRef == "" {
			return nil, errors.NewError().Messagef("image_ref is required for push operation").WithLocation().Build()
		}
	...
	case "tag":
		sourceRef := po.safeStringFromMap(args, "source_ref")
		targetRef := po.safeStringFromMap(args, "target_ref")
		if sourceRef == "" || targetRef == "" {
			return nil, errors.NewError().Messagef("source_ref and target_ref are required for tag operation").WithLocation().Build()
		}
	...
	default:
		err = errors.NewError().Messagef("unsupported operation type: %s", operationType).WithLocation().Build()
	}
...
// safeStringFromMap safely extracts a string value from a map, returning empty string if not found or wrong type
func (po *PerformanceOptimizer) safeStringFromMap(args map[string]interface{}, key string) string {
	if value, exists := args[key]; exists {
		...
		po.logger.Warn().
			...
			Msg("Expected string value but got different type")
	}
	return ""
}
