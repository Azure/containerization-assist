...
func (rm *RecoveryManager) CreateRecoveryPoint(ctx context.Context, sessionID string, metadata map[string]interface{}) (*RecoveryPoint, error) {
	...
	// Get session state
	sessionState, err := rm.sessionManager.GetSession(ctx, sessionID)
	if err != nil {
		return nil, errors.NewError().Message("failed to get session state").Cause(err).WithLocation().Build()
	}

	// Convert ActiveJobs from map to slice of job IDs
	activeJobsList := make([]string, 0, len(sessionState.ActiveJobs))
	for jobID := range sessionState.ActiveJobs {
		activeJobsList = append(activeJobsList, jobID)
	}
	...
	recoveryPoint := &RecoveryPoint{
		...
		CompletedTools: []string{}, // Extract from stage history if needed
		Metadata:       metadata,
	}
...
func (rm *RecoveryManager) RecoverSession(ctx context.Context, sessionID string, recoveryPointID string) (*RecoveryOperation, error) {
	...
	if _, exists := rm.activeRecovery[sessionID]; exists {
		return nil, errors.NewError().Messagef("recovery already in progress for session: %s", sessionID).WithLocation().Build()
	}

	recoveryPoint, exists := rm.recoveryPoints[recoveryPointID]
	if !exists {
		return nil, errors.NewError().Messagef("recovery point not found: %s", recoveryPointID).WithLocation().Build()
	}

	if !rm.validateRecoveryPoint(recoveryPoint) {
		return nil, errors.NewError().Messagef("recovery point integrity check failed").WithLocation().Build()
	}
...
func (rm *RecoveryManager) GetRecoveryStatus(sessionID string) (*RecoveryOperation, error) {
	...
	if !exists {
		return nil, errors.NewError().Messagef("no active recovery operation for session: %s", sessionID).Build()
	}
...
func (rm *RecoveryManager) InitiateFailover(ctx context.Context, reason string) error {
	...
	if rm.isFailoverActive {
		return errors.NewError().Messagef("failover already in progress").WithLocation().Build()
	}

	failoverNode := rm.selectFailoverNode()
	if failoverNode == nil {
		return errors.NewError().Messagef("no available failover nodes").Build()
	}
...
func (rm *RecoveryManager) stepRestoreSessionState(ctx context.Context, recoveryOp *RecoveryOperation) error {
	...
	return rm.sessionManager.UpdateSession(context.Background(), recoveryOp.SessionID, func(sess *session.SessionState) error {
...
func (rm *RecoveryManager) stepRestoreActiveJobs(ctx context.Context, recoveryOp *RecoveryOperation) error {
	...
	for _, jobID := range recoveryOp.RecoveryPoint.ActiveJobs {
		err := rm.sessionManager.UpdateSession(context.Background(), recoveryOp.SessionID, func(sess *session.SessionState) error {
...
func (rm *RecoveryManager) stepVerifyRecovery(ctx context.Context, recoveryOp *RecoveryOperation) error {
	...
	sessionState, err := rm.sessionManager.GetSession(ctx, recoveryOp.SessionID)
	if err != nil {
		return fmt.Errorf("failed to verify recovery: %w", err)
	}

	// Basic verification - in production, this would be more comprehensive
	if len(sessionState.ActiveJobs) != len(recoveryOp.RecoveryPoint.ActiveJobs) {
