...
import (
	...
	validationCore "github.com/Azure/container-kit/pkg/mcp/domain/validation"
)
		...
		if dockerResult, ok := resultStruct.(map[string]interface{}); ok {
			...
			if content != "" {
				...
				if validationData, ok := dockerResult["validation"].(map[string]interface{}); ok {
					...
					if errors, ok := validationData["github.com/Azure/container-kit/pkg/mcp/domain/errors"].([]interface{}); ok {
