...
func StartMCPServerForTest(t *testing.T) (StdioMCPClient, error) {
	...
	if err := buildCmd.Run(); err != nil {
		return nil, errors.NewError().Message("failed to build MCP server").Cause(err).Build()
	}
...
func (c *stdioMCPClient) ListTools() ([]ToolInfo, error) {
	...
	if err := c.sendRequest("initialize", initParams); err != nil {
		return nil, errors.NewError().Message("failed to send initialize request").Cause(err).Build()
	}

	initResp, err := c.readResponse()
	if err != nil {
		return nil, errors.NewError().Message("failed to read initialize response").Cause(err).Build()
	}

	if initResp.Error != nil {
		return nil, errors.NewError().Messagef("initialize error: %s (code: %d)", initResp.Error.Message, initResp.Error.Code).Build()
	}

	if err := c.sendNotification("initialized", nil); err != nil {
		return nil, errors.NewError().Message("failed to send initialized notification").Cause(err).Build()
	}

	if err := c.sendRequest("tools/list", nil); err != nil {
		return nil, errors.NewError().Message("failed to send tools/list request").Cause(err).WithLocation(
	...
	for {
		...
		if err != nil {
			return nil, errors.NewError().Message("failed to read response").Cause(err).WithLocation(
	...
	if resp.Error != nil {
		return nil, errors.NewError().Messagef("tools/list error: %s (code: %d)", resp.Error.Message, resp.Error.Code).Build()
	}

	resultMap, ok := resp.Result.(map[string]interface{})
	if !ok {
		return nil, errors.NewError().Messagef("unexpected result type: %T", resp.Result).Build()
	}

	toolsInterface, ok := resultMap["tools"]
	if !ok {
		return nil, errors.NewError().Messagef("no 'tools' field in response").Build()
	}

	toolsArray, ok := toolsInterface.([]interface{})
	if !ok {
		return nil, errors.NewError().Messagef("tools field is not an array: %T", toolsInterface).Build()
	}
	...
	for _, toolInterface := range toolsArray {
		...
		if err != nil {
			return nil, errors.NewError().Message("failed to marshal tool").Cause(err).Build()
		}

		var tool ToolInfo
		if err := json.Unmarshal(toolBytes, &tool); err != nil {
			return nil, errors.NewError().Message("failed to unmarshal tool schema").Cause(err).Build()
		}
...
func (c *stdioMCPClient) CallTool(name string, args map[string]interface{}) (map[string]interface{}, error) {
	...
	if err := c.sendRequest("tools/call", params); err != nil {
		return nil, errors.NewError().Message("failed to send tools/call request").Cause(err).Build()
	}

	resp, err := c.readResponse()
	if err != nil {
		return nil, errors.NewError().Message("failed to read response").Cause(err).Build()
	}

	if resp.Error != nil {
		return nil, errors.NewError().Messagef("tools/call error: %s (code: %d)", resp.Error.Message, resp.Error.Code).Build()
	}
