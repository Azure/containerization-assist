...
import (
	...
	di "github.com/Azure/container-kit/pkg/mcp/container"
...
func NewTestServer() (*TestServer, error) {
	...
	// Create DI container configuration for testing
	diConfig := &di.Config{
		DatabasePath: filepath.Join(tempDir, "test-sessions.db"),
		LogLevel:     "error", // Reduce logging noise in tests
		Environment:  "test",
	}

	// Create DI container
	containerInstance, err := di.NewContainer(diConfig)
	if err != nil {
		os.RemoveAll(tempDir)
		return nil, err
	}

	// Initialize MCP server with service-based architecture
	...
	mcpServer, err := server.NewServerWithServices(ctx, config, containerInstance)
	if err != nil {
