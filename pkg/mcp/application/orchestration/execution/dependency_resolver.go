...
func (dr *DefaultDependencyResolver) ResolveDependencies(stages []WorkflowStage) ([][]WorkflowStage, error) {
	...
	for len(completed) < len(stages) {
		...
		if len(currentGroup) == 0 {
			...
			return nil, errors.NewError().Messagef("circular dependency detected or missing dependencies for stages: %v", remaining).WithLocation(
...
func (dr *DefaultDependencyResolver) ValidateDependencies(stages []WorkflowStage) error {
	...
	for _, stage := range stages {
		if stageMap[stage.Name] {
			return errors.NewError().Messagef("duplicate stage name: %s", stage.Name).Build()
		}
	...
	for _, stage := range stages {
		for _, dep := range stage.DependsOn {
			if !stageMap[dep] {
				return errors.NewError().Messagef("stage %s depends on non-existent stage: %s", stage.Name, dep).
	...
	for _, stage := range stages {
		if !visited[stage.Name] {
			if dr.hasCycle(stage.Name, stages, visited, recursionStack) {
				return errors.NewError().Messagef("circular dependency detected involving stage: %s", stage.Name).Build()
			}
		}
	}
