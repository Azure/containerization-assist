...
func (o *Operations) AnalyzeRepositoryTyped(_ context.Context, sessionID string, params domain.AnalyzeParams) (*domain.AnalyzeResult, error) {
	if sessionID == "" {
		return nil, errors.NewError().Messagef("session ID is required").WithLocation().Build()
	}

	// Check for repository path in either Path or RepositoryPath fields
	...
	o.logger.Info("Analyzing repository", "session_id", sessionID, "repository_path", repoPath)
	...
	return &domain.AnalyzeResult{
		...
		RepositoryInfo: domain.RepositoryInfo{
			Path:      repoPath,
			Language:  "detected",
			Framework: "unknown",
		},
		BuildRecommendations: domain.BuildRecommendations{
			OptimizationSuggestions: []domain.Recommendation{
...
func (o *Operations) ValidateDockerfileTyped(_ context.Context, sessionID string, params domain.ValidateParams) (*domain.ConsolidatedValidateResult, error) {
	if sessionID == "" {
		return nil, errors.NewError().Messagef("session ID is required").WithLocation().Build()
	}
	...
	return &domain.ConsolidatedValidateResult{
		BaseToolResponse: domain.BaseToolResponse{
			Success:   true,
...
func (o *Operations) ScanSecurityTyped(_ context.Context, sessionID string, params domain.ConsolidatedScanParams) (*domain.ScanResult, error) {
	if sessionID == "" {
		return nil, errors.NewError().Messagef("session ID is required").WithLocation().Build()
	}
	if params.ImageRef == "" {
...
func (o *Operations) ScanSecretsTyped(_ context.Context, sessionID string, params domain.ScanSecretsParams) (*domain.ScanSecretsResult, error) {
	if sessionID == "" {
		return nil, errors.NewError().Messagef("session ID is required").WithLocation().Build()
	}
	if params.Path == "" {
	...
	return &domain.ScanSecretsResult{
		BaseToolResponse: domain.BaseToolResponse{
			Success:   true,
		...
		SecretsFound: 0,
		FilesScanned: 1, // Indicate at least one file was scanned
		Files:        []string{},
		Secrets:      []string{},
	}, nil
}
