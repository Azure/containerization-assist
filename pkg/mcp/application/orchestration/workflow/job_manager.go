...
func NewJobManagerWithServices(serviceContainer services.ServiceContainer, logger *slog.Logger) (*JobManager, error) {
	...
	config := JobManagerConfig{
		MaxWorkers: 5,
		JobTTL:     1 * time.Hour,
		Logger:     logger.With("component", "job_manager"), // Now using slog
	}
...
func (jm *JobManager) StartJob(jobID string, executor func(context.Context, *AsyncJobInfo) error) error {
	...
	go func() {
		...
		if err != nil {
			jm.logger.Error("Failed to update job status to running", "error", err, "job_id", jobID)
			return
		}
		...
		if err != nil {
			jm.logger.Error("Failed to get job for execution", "error", err, "job_id", jobID)
			return
		}
		...
		if err := jm.UpdateJob(jobID, func(job *AsyncJobInfo) {
			...
			jm.logger.Error("Failed to update job status after execution", "error", err, "job_id", jobID)
		}

		jm.logger.Info("Job execution finished",
			"job_id", jobID,
			"status", string(job.Status),
			"error", execErr)
	}()
...
func (jm *JobManager) Stop() {
	...
	for jobID, job := range jm.jobs {
		if job.Status == sessiontypes.JobStatusPending {
			...
			jm.logger.Info("Cancelled pending job due to shutdown", "job_id", jobID)
		}
	}
