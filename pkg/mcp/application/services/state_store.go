...
func NewStateStoreImpl(stateManager *state.UnifiedStateManager) StateStore {
...
func (s *stateStoreImpl) Save(ctx context.Context, key string, st interface{}) error {
	...
	return s.stateManager.SetState(ctx, state.StateTypeGlobal, key, st)
}

func (s *stateStoreImpl) Load(ctx context.Context, key string) (interface{}, error) {
	// Implement using UnifiedStateManager methods
	return s.stateManager.GetState(ctx, state.StateTypeGlobal, key)
}

func (s *stateStoreImpl) Delete(ctx context.Context, key string) error {
	// Implement using UnifiedStateManager methods
	return s.stateManager.DeleteState(ctx, state.StateTypeGlobal, key)
}
