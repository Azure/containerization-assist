...
func extractDatabaseTypes(databaseFiles []string) []string {
	...
	for _, file := range databaseFiles {
		switch {
		case commonUtils.Contains(strings.ToLower(file), "postgres") || commonUtils.Contains(strings.ToLower(file), "postgresql"):
			types = append(types, "postgresql")
		case commonUtils.Contains(strings.ToLower(file), "mysql"):
			types = append(types, "mysql")
		case commonUtils.Contains(strings.ToLower(file), "mongo"):
			types = append(types, "mongodb")
		case commonUtils.Contains(strings.ToLower(file), "redis"):
			types = append(types, "redis")
		case commonUtils.Contains(strings.ToLower(file), "sqlite"):
