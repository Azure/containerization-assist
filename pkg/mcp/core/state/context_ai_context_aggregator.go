...
func (a *AIContextAggregator) getStateSnapshot(ctx context.Context, sessionID string) (*StateSnapshot, error) {
	...
	workflowIDs, _ := a.stateManager.GetState(ctx, StateTypeWorkflow, sessionID)
	if workflowList, ok := workflowIDs.([]string); ok {
		for _, wfID := range workflowList {
			if wfState, err := a.stateManager.GetState(ctx, StateTypeWorkflow, wfID); err == nil {
				snapshot.WorkflowStates[wfID] = wfState
			}
		}
	}

	toolStateIDs, _ := a.stateManager.GetState(ctx, StateTypeTool, sessionID)
	if toolList, ok := toolStateIDs.([]string); ok {
		for _, toolID := range toolList {
			if toolState, err := a.stateManager.GetState(ctx, StateTypeTool, toolID); err == nil {
...
func (a *AIContextAggregator) getRecentEvents(ctx context.Context, sessionID string) []*Event {
	...
	stateEvents, err := a.stateManager.GetStateHistory(ctx, StateTypeSession, sessionID, 50)
	if err == nil {
