...
type GenerateDockerfileArgs struct {
	BaseImage          string            `json:"base_image,omitempty" validate:"omitempty,docker_image" description:"Override detected base image"`
	Template           string            `json:"template,omitempty" validate:"omitempty,oneof=go node python java rust php ruby dotnet golang nodejs js generic" description:"Use specific template (go, node, python, etc.)"`
	Optimization       string            `json:"optimization,omitempty" validate:"omitempty,oneof=size speed security balanced" description:"Optimization level (size, speed, security)"`
	...
	Platform           string            `json:"platform,omitempty" validate:"omitempty,platform" description:"Target platform (e.g., linux/amd64)"`
	SessionID          string            `json:"session_id,omitempty" validate:"required,session_id" description:"Session ID for context"`
