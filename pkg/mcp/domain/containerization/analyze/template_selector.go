...
var languageCompatibilityMap = map[string][]string{
	"go":     {"go", "golang"},
...
func (ts *TemplateSelector) scoreLanguageMatch(templateType, language string) int {
	if compatibleLanguages, exists := languageCompatibilityMap[templateType]; exists {
...
// configFileScores maps template types to their configuration files and scores
var configFileScores = map[string]map[string]int{
	"go": {
		"go.mod": 20,
		"go.sum": 20,
	},
...
func (ts *TemplateSelector) getConfigFileScore(templateType, file string) int {
	if scores, exists := configFileScores[templateType]; exists {
		if score, found := scores[file]; found {
			return score
		}

		// Handle .NET project files with suffix matching
		if templateType == "dotnet" && ts.isDotNetProjectFile(file) {
			return 20
		}
	}
	return 0
