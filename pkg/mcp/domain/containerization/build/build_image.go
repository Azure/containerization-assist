...
func (t *BuildImageTool) ExecuteTyped(ctx context.Context, args BuildImageArgs) (*BuildImageResult, error) {
	...
	if args.AsyncBuild || buildTimeout > 2*time.Minute {
		...
		go func() {
			...
			asyncErr := t.executeAsyncBuild(ctx, opstypes.AsyncBuildConfig{
				Args:          args,
				PipelineState: pipelineState,
				DockerStage:   dockerStage,
...
func (t *BuildImageTool) executeAsyncBuild(ctx context.Context, config opstypes.AsyncBuildConfig) error {
	// Safe type assertions with proper error handling
	args, ok := config.Args.(BuildImageArgs)
	if !ok {
		return errors.NewError().
			Code("TYPE_ASSERTION_FAILED").
			Message("Failed to convert args to BuildImageArgs").
			Type(errors.ErrTypeValidation).
			Severity(errors.SeverityMedium).
			Context("args_type", fmt.Sprintf("%T", config.Args)).
			Suggestion("Ensure the args are of type BuildImageArgs").
			WithLocation().
			Build()
	}

	pipelineState, ok := config.PipelineState.(*pipeline.PipelineState)
	if !ok {
		return errors.NewError().
			Code("TYPE_ASSERTION_FAILED").
			Message("Failed to convert pipeline state to PipelineState").
			Type(errors.ErrTypeValidation).
			Severity(errors.SeverityMedium).
			Context("pipeline_state_type", fmt.Sprintf("%T", config.PipelineState)).
			Suggestion("Ensure the pipeline state is of type PipelineState").
			WithLocation().
			Build()
	}

	dockerStage, ok := config.DockerStage.(*dockerstage.DockerStage)
	if !ok {
		return errors.NewError().
			Code("TYPE_ASSERTION_FAILED").
			Message("Failed to convert docker stage to DockerStage").
			Type(errors.ErrTypeValidation).
			Severity(errors.SeverityMedium).
			Context("docker_stage_type", fmt.Sprintf("%T", config.DockerStage)).
			Suggestion("Ensure the docker stage is of type DockerStage").
			WithLocation().
			Build()
	}

	runnerOptions, ok := config.RunnerOptions.(pipeline.RunnerOptions)
	if !ok {
		return errors.NewError().
			Code("TYPE_ASSERTION_FAILED").
			Message("Failed to convert runner options to RunnerOptions").
			Type(errors.ErrTypeValidation).
			Severity(errors.SeverityMedium).
			Context("runner_options_type", fmt.Sprintf("%T", config.RunnerOptions)).
			Suggestion("Ensure the runner options are of type RunnerOptions").
			WithLocation().
			Build()
	}
...
func (t *BuildImageTool) Execute(ctx context.Context, args interface{}) (interface{}, error) {
	...
	case map[string]interface{}:
		...
		if err != nil {
			return AtomicBuildImageArgs{}, errors.NewError().Message("failed to marshal arguments").Cause(err).Build()
		}
		if err := json.Unmarshal(jsonData, &buildArgs); err != nil {
			return AtomicBuildImageArgs{}, errors.NewError().Message("failed to unmarshal arguments").Cause(err).Build()
		}
	default:
		return AtomicBuildImageArgs{}, errors.NewError().Messagef("unsupported argument type: %T", args).Build()
	}
