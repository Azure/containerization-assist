...
type BuildValidator interface {
	...
	ValidateDockerfile(dockerfilePath string) (*BuildValidationResult, error)
	// ValidateBuildContext checks if the build context is valid
	ValidateBuildContext(ctx BuildContext) (*BuildValidationResult, error)
...
// BuildValidationResult is now an alias to the unified validation framework for build domain
type BuildValidationResult = core.BuildResult

// SecurityBuildValidationResult is now an alias to the unified security validation framework
type SecurityValidationResult = core.SecurityResult

// ValidationError and ValidationWarning are aliases to the unified types for backward compatibility
...
// LegacySecurityBuildValidationResult contains security validation results (legacy)
type LegacySecurityValidationResult struct {
	Secure               bool
