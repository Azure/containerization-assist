...
func NewScannerRegistry(logger *slog.Logger) *ScannerRegistry {
	return &ScannerRegistry{
		...
		logger:   logger.With("component", "scanner_registry"),
	}
}
...
func (r *ScannerRegistry) Register(scanner SecretScanner) {
	...
	r.logger.Debug("Secret scanner registered", "scanner", scanner.GetName())
}
...
func (r *ScannerRegistry) ScanWithAllApplicable(ctx context.Context, config ScanConfig) (*CombinedScanResult, error) {
	...
	r.logger.Info("Running applicable secret scanners", "scanners", len(applicable))

	for _, scanner := range applicable {
		r.logger.Debug("Running secret scanner", "scanner", scanner.GetName())
		scanResult, err := scanner.Scan(ctx, config)
		if err != nil {
			r.logger.Error("Scanner failed", "error", err, "scanner", scanner.GetName())
			continue
		}
...
func (r *ScannerRegistry) generateSummary(result *CombinedScanResult) map[string]interface{} {
	...
	for _, secret := range result.AllSecrets {
		...
		if bySeverity, ok := summary["by_severity"].(map[string]int); ok {
			bySeverity[string(secret.Severity)]++
		}
