...
// TokenUsageInfo captures AI model token consumption for cost tracking
type TokenUsageInfo struct {
	ModelName     string        `json:"model_name"`     // e.g., "gpt-4o", "claude-3-5-sonnet"
	Provider      string        `json:"provider"`       // e.g., "openai", "anthropic", "azure"
	...
	Latency       time.Duration `json:"latency"`        // Time taken for the request
	RequestID     string        `json:"request_id"`     // AI provider's request ID
	Timestamp     time.Time     `json:"timestamp"`      // When the request was made
	CacheHit      bool          `json:"cache_hit"`      // Whether response was cached
	ContextLength int64         `json:"context_length"` // Total context window used
}
...
type ConversationResponse struct {
	...
	Stage         string                 `json:"stage"`
	TokenUsage    *TokenUsageInfo        `json:"token_usage,omitempty"` // Track AI model token consumption
