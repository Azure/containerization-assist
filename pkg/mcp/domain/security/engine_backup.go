...
func (ve *Engine) ValidateStruct(ctx context.Context, data interface{}) *Result {
	...
	for _, rule := range ve.rules {
		...
		if err := ve.applyRuleToData(rule, data, ""); err != nil {
...
func (ve *Engine) applyRuleToData(rule Rule, data interface{}, pathPrefix string) *Error {
	...
	for _, constraint := range rule.Constraints {
		if err := ve.applyConstraintToValue(constraint, fieldValue, fieldPath); err != nil {
			err.Path = fieldPath
			return err
		}
	}
...
func (ve *Engine) applyConstraintToValue(constraint FieldConstraint, fieldValue interface{}, fieldPath string) *Error {
	...
	return validator.Validate(fieldValue, fieldPath, constraint.Value)
}
...
func (ve *Engine) extractFieldValue(fieldPath string, data interface{}, pathPrefix string) (interface{}, string) {
