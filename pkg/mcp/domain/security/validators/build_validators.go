...
func (dv *DockerfileValidator) ValidateDockerfile(dockerfileContent string) *domain.BuildValidationResult {
	result := domain.NewBuildResult()
	...
	if strings.TrimSpace(dockerfileContent) == "" {
		result.Valid = false
		emptyError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateInstruction(instruction, line string, lineNum int, result *domain.BuildValidationResult) {
	switch instruction {
	case "FROM":
	...
	default:
		...
		if !found {
			unknownError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateFromInstruction(line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		fromError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateRunInstruction(line string, lineNum int, result *domain.BuildValidationResult) {
...
func (dv *DockerfileValidator) validateCopyAddInstruction(instruction, line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 3 {
		copyError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateExposeInstruction(line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		exposeError := &domain.ValidationError{
	...
	for _, port := range parts[1:] {
		if !portRegex.MatchString(port) {
			portError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateUserInstruction(line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		userError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateWorkdirInstruction(line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		workdirError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateCmdEntrypointInstruction(instruction, line string, lineNum int, result *domain.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		cmdError := &domain.ValidationError{
...
func (dv *DockerfileValidator) validateStructure(instructions []string, result *domain.BuildValidationResult) {
...
func (dv *DockerfileValidator) addGeneralSuggestions(dockerfileContent string, result *domain.BuildValidationResult) {
