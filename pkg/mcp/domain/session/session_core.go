...
// DefaultStoreFactory is the default store factory (must be set by the application)
var DefaultStoreFactory StoreFactory
...
func NewSessionManager(config SessionManagerConfig) (*SessionManager, error) {
	...
	if config.StorePath != "" {
		...
		boltStore, err := DefaultStoreFactory(context.Background(), config.StorePath)
		if err != nil {
