...
func (bc *BuildConfig) Validate() error {
	if bc.ImageName == "" {
		return NewRichValidationError("image_name", "required field cannot be empty")
	}

	if bc.DockerfilePath == "" {
		return NewRichValidationError("dockerfile_path", "required field cannot be empty")
	}

	if bc.ContextPath == "" {
		return NewRichValidationError("context_path", "required field cannot be empty")
	}

	if bc.Timeout < time.Second {
		return NewRichValidationError("timeout", "must be at least 1 second")
	}

	if bc.Retries < 0 || bc.Retries > 10 {
		return NewRichValidationError("retries", "must be between 0 and 10")
	}
