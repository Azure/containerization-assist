...
func (c *ServerConfig) Validate() error {
	if c.WorkspaceDir == "" {
		return NewConfigValidationError("workspace_dir", "workspace directory is required")
	}

	if c.StorePath == "" {
		return NewConfigValidationError("store_path", "store path is required")
	}

	if c.MaxSessions < 1 {
		return NewConfigValidationError("max_sessions", "max sessions must be at least 1")
	}

	if c.SessionTTL <= 0 {
		return NewConfigValidationError("session_ttl", "session TTL must be positive")
	}

	if c.TransportType != "stdio" && c.TransportType != "http" {
		return NewConfigValidationError("transport_type", "transport type must be 'stdio' or 'http'")
	}

	if c.TransportType == "http" {
		if c.HTTPPort < 1 || c.HTTPPort > 65535 {
			return NewConfigValidationError("http_port", "HTTP port must be between 1 and 65535")
		}
	}

	if c.MaxWorkers < 1 {
		return NewConfigValidationError("max_workers", "max workers must be at least 1")
	}

	if c.CleanupInterval <= 0 {
		return NewConfigValidationError("cleanup_interval", "cleanup interval must be positive")
	}

	if c.MaxConcurrentRequests < 1 {
		return NewConfigValidationError("max_concurrent_requests", "max concurrent requests must be at least 1")
	}
...
func (c *ServerConfig) GetWorkspaceDir() (string, error) {
	if err := os.MkdirAll(c.WorkspaceDir, 0755); err != nil {
		return "", NewConfigValidationError("workspace_dir", "failed to create workspace directory: "+err.Error())
	}
...
func (c *ServerConfig) GetStorePath() (string, error) {
	if err := os.MkdirAll(c.StorePath, 0755); err != nil {
		return "", NewConfigValidationError("store_path", "failed to create store directory: "+err.Error())
	}
