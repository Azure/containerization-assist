...
type HTTPValidationResponse struct {
	Success bool                  `json:"success"`
	Errors  []HTTPValidationError `json:"github.com/Azure/container-kit/pkg/mcp/domain/errors"`
	Message string                `json:"message"`
}
...
// TypedServerInterface represents a type-safe server interface
type TypedServerInterface interface {
	GetTypedToolOrchestrator() TypedOrchestrator
}

// TypedOrchestrator represents a type-safe orchestrator interface
type TypedOrchestrator interface {
	GetTypedToolMetadata(string) (*HTTPToolMetadata, error)
}

// TypedMessageHandler represents a typed message handler
type TypedMessageHandler interface {
	HandleMessage(message *ToolExecutionRequest) (*ToolExecutionResponse, error)
}

// TypedMessageTransport represents a type-safe message transport interface
type TypedMessageTransport interface {
	SendTypedMessage(message *ToolExecutionRequest) (*ToolExecutionResponse, error)
	ReceiveTypedMessage() (*ToolExecutionRequest, error)
}
