// Package health provides health monitoring for MCP services
package health
...
// Report represents an overall health report
type Report struct {
	Status    Status            `json:"status"`
...
// Monitor manages health checks for MCP services
type Monitor struct {
	mu        sync.RWMutex
	logger    *slog.Logger
...
// BasicChecker is a simple health checker implementation
type BasicChecker struct {
	name    string
	checkFn func(ctx context.Context) (Status, string, map[string]string)
}

// NewBasicChecker creates a basic health checker
