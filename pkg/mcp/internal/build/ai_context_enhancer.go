...
func NewAIContextEnhancer(contextSharer *DefaultContextSharer, logger zerolog.Logger) *AIContextEnhancer {
	return &AIContextEnhancer{
		contextSharer: contextSharer,
		logger:        logger.With().Str("component", "ai_context_enhancer").Logger(),
	}
}
...
func (e *AIContextEnhancer) generateRecommendations(aiContext *AIToolContext) *AIRecommendations {
	...
	} else if aiContext.ErrorContext != nil {
		if aiContext.ErrorContext.FixProbability == "high" {
			recommendations.NextBestActions = append(recommendations.NextBestActions, AIActionRecommendation{
