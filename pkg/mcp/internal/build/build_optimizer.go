...
func NewBuildOptimizer(logger zerolog.Logger) *BuildOptimizer {
	return &BuildOptimizer{
		logger:           logger.With().Str("component", "build_optimizer").Logger(),
...
func (o *BuildOptimizer) OptimizeBuild(ctx context.Context, dockerfilePath string, buildContext string) (*OptimizationResult, error) {
	o.logger.Info().
		...
		Msg("Starting build optimization analysis")
	...
	if err != nil {
		o.logger.Warn().Err(err).Msg("Failed to analyze Dockerfile")
	} else {
	...
	if err != nil {
		o.logger.Warn().Err(err).Msg("Failed to analyze build context")
	} else {
...
func NewLayerOptimizer(logger zerolog.Logger) *LayerOptimizer {
	return &LayerOptimizer{
		logger: logger.With().Str("component", "layer_optimizer").Logger(),
	}
}
...
func NewContextOptimizer(logger zerolog.Logger) *ContextOptimizer {
	return &ContextOptimizer{
		logger: logger.With().Str("component", "context_optimizer").Logger(),
	}
}
