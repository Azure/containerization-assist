...
type SecurityChecksProvider interface {
	CheckForRootUser(lines []string, result *ValidationResult)
	CheckForSecrets(lines []string, result *ValidationResult, patterns []*regexp.Regexp)
	CheckForSensitivePorts(lines []string, result *ValidationResult)
	CheckPackagePinning(lines []string, result *ValidationResult)
	CheckForSUIDBindaries(lines []string, result *ValidationResult)
	CheckBaseImageSecurity(lines []string, result *ValidationResult, trustedRegistries []string)
	CheckForInsecureDownloads(lines []string, result *ValidationResult)
}
...
func (v *SecurityValidator) calculateSecurityScore(result *ValidationResult) float64 {
...
func (v *SecurityValidator) determineRiskLevel(result *ValidationResult) string {
...
func (v *SecurityValidator) generateRecommendations(result *ValidationResult) []string {
...
// containsSecret checks if a line contains potential secrets
func containsSecret(line string, patterns []*regexp.Regexp) bool {
	for _, pattern := range patterns {
