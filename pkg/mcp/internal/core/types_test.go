...
func TestNewBaseResponse(t *testing.T) {
	...
	// Capture time before creating response
	beforeTime := time.Now()

	response := NewBaseResponse(tool, sessionID, dryRun)

	// Capture time after creating response
	afterTime := time.Now()
...
func TestBaseToolResponse_Structure(t *testing.T) {
	// Test that BaseToolResponse can be created and has correct fields
...
func TestBaseToolArgs_Structure(t *testing.T) {
	// Test that BaseToolArgs can be created and has correct fields
	args := BaseToolArgs{
		DryRun:    true,
		SessionID: "test-session",
	}
...
func TestResourceRequests_Structure(t *testing.T) {
	// Test that ResourceRequests can be created with various values
...
func TestSecretRef_Structure(t *testing.T) {
	// Test that SecretRef can be created and has correct fields
	secret := SecretRef{
		Name: "my-secret",
		Key:  "password",
		Env:  "DB_PASSWORD",
...
func TestPortForward_Structure(t *testing.T) {
	// Test that PortForward can be created and has correct fields
...
func TestResourceUtilization_Structure(t *testing.T) {
	// Test that ResourceUtilization can be created and has correct fields
	utilization := ResourceUtilization{
		CPU:         45.6,
		Memory:      78.2,
		Disk:        23.1,
		DiskFree:    1024 * 1024 * 1024, // 1GB
		LoadAverage: 1.5,
	}
...
func TestServiceHealth_Structure(t *testing.T) {
	// Test that ServiceHealth can be created and has correct fields
	now := time.Now()
...
func TestServiceHealth_WithError(t *testing.T) {
	// Test that ServiceHealth can handle error conditions
	now := time.Now()
	health := ServiceHealth{
		Status:    "unhealthy",
		LastCheck: now,
	...
	assert.Zero(t, health.ResponseTime) // Should be zero when there's an error
}
