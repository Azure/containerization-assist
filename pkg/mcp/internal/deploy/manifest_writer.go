...
func (w *Writer) EnsureDirectory(path string) error {
	if err := os.MkdirAll(path, 0755); err != nil {
		return mcptypes.NewRichError("DIRECTORY_CREATION_FAILED", fmt.Sprintf("failed to create directory %s: %v", path, err), "filesystem_error")
	}
...
func (w *Writer) WriteFile(filePath string, content []byte) error {
	if err := os.WriteFile(filePath, content, 0644); err != nil {
		return mcptypes.NewRichError("FILE_WRITE_FAILED", fmt.Sprintf("failed to write file %s: %v", filePath, err), "filesystem_error")
	}
...
func (w *Writer) WriteDeploymentTemplate(manifestPath string, opts GenerationOptions) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read deployment template: %v", err), "template_error")
	}

	// Apply template substitutions
	processed, err := w.processTemplate("deployment", string(templateContent), opts)
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process deployment template: %v", err), "template_error")
	}

	if err := w.WriteFile(deploymentPath, []byte(processed)); err != nil {
		return mcptypes.NewRichError("TEMPLATE_WRITE_FAILED", fmt.Sprintf("failed to write deployment template: %v", err), "template_error")
	}
...
func (w *Writer) WriteServiceTemplate(manifestPath string, opts GenerationOptions) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read service template: %v", err), "template_error")
	}

	// Apply template substitutions
	processed, err := w.processTemplate("service", string(templateContent), opts)
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process service template: %v", err), "template_error")
	}

	if err := w.WriteFile(servicePath, []byte(processed)); err != nil {
		return mcptypes.NewRichError("TEMPLATE_WRITE_FAILED", fmt.Sprintf("failed to write service template: %v", err), "template_error")
	}
...
func (w *Writer) WriteConfigMapTemplate(manifestPath string, opts GenerationOptions) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read configmap template: %v", err), "template_error")
	}

	// Apply template substitutions
	processed, err := w.processTemplate("configmap", string(templateContent), opts)
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process configmap template: %v", err), "template_error")
	}

	if err := w.WriteFile(configMapPath, []byte(processed)); err != nil {
		return mcptypes.NewRichError("TEMPLATE_WRITE_FAILED", fmt.Sprintf("failed to write configmap template: %v", err), "template_error")
	}
...
func (w *Writer) WriteIngressTemplate(manifestPath string, opts GenerationOptions) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read ingress template: %v", err), "template_error")
	}

	// Apply template substitutions
	processed, err := w.processTemplate("ingress", string(templateContent), opts)
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process ingress template: %v", err), "template_error")
	}

	if err := w.WriteFile(ingressPath, []byte(processed)); err != nil {
		return mcptypes.NewRichError("TEMPLATE_WRITE_FAILED", fmt.Sprintf("failed to write ingress template: %v", err), "template_error")
	}
...
func (w *Writer) WriteSecretTemplate(manifestPath string, opts GenerationOptions) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read secret template: %v", err), "template_error")
	}

	// Apply template substitutions
	processed, err := w.processTemplate("secret", string(templateContent), opts)
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process secret template: %v", err), "template_error")
	}

	if err := w.WriteFile(secretPath, []byte(processed)); err != nil {
		return mcptypes.NewRichError("TEMPLATE_WRITE_FAILED", fmt.Sprintf("failed to write secret template: %v", err), "template_error")
	}
...
func (w *Writer) WriteManifestFromTemplate(filePath, templatePath string, data interface{}) error {
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_READ_FAILED", fmt.Sprintf("failed to read template %s: %v", templatePath, err), "template_error")
	}
	...
	if opts, ok := data.(GenerationOptions); ok {
		...
		if err != nil {
			return mcptypes.NewRichError("TEMPLATE_PROCESSING_FAILED", fmt.Sprintf("failed to process template: %v", err), "template_error")
		}
	...
	if err != nil {
		return mcptypes.NewRichError("TEMPLATE_PARSE_FAILED", fmt.Sprintf("failed to parse template: %v", err), "template_error")
	}

	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, data); err != nil {
		return mcptypes.NewRichError("TEMPLATE_EXECUTION_FAILED", fmt.Sprintf("failed to execute template: %v", err), "template_error")
	}
