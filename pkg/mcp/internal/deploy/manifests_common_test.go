...
func TestValidationResult(t *testing.T) {
	result := ValidationResult{
		...
		Valid:        true,
		Errors:       []string{},
		Warnings:     []string{"Consider using specific image tag"},
	}

	if result.ManifestName != "deployment.yaml" {
		t.Errorf("Expected ManifestName to be 'deployment.yaml', got '%s'", result.ManifestName)
	}
	if !result.Valid {
