...
// GetMergeReadiness provides merge readiness recommendation
func (qm *QualityMonitor) GetMergeReadiness(ctx context.Context) (MergeReadiness, error) {
	qm.logger.Info().Msg("Evaluating merge readiness")

	gates, err := qm.ValidateQualityGates(ctx)
	if err != nil {
		return MergeReadiness{}, fmt.Errorf("failed to validate quality gates: %v", err)
	}
	...
	readiness := MergeReadiness{
		Timestamp:      time.Now(),
		OverallReady:   true,
		TeamReadiness:  make(map[string]TeamReadiness),
		QualityGates:   gates,
		Recommendation: "Ready to merge",
	}

	// Check each team's readiness
	for teamName, team := range report.Teams {
		teamReady := TeamReadiness{
			TeamName:    teamName,
			Ready:       team.Status == "GREEN",
			Blockers:    []string{},
			Coverage:    team.TestCoverage,
			LintIssues:  team.LintIssues,
			Performance: team.PerformanceP95,
		}

		// Add blockers
		if team.TestCoverage < qm.thresholds.MinTestCoverage {
			teamReady.Blockers = append(teamReady.Blockers,
				fmt.Sprintf("Test coverage %.1f%% below %.1f%% threshold",
					team.TestCoverage, qm.thresholds.MinTestCoverage))
		}

		if team.LintIssues > qm.thresholds.MaxLintIssues {
			teamReady.Blockers = append(teamReady.Blockers,
				fmt.Sprintf("Lint issues %d exceed %d threshold",
					team.LintIssues, qm.thresholds.MaxLintIssues))
		}

		if team.PerformanceP95 > qm.thresholds.MaxPerformanceP95 {
			teamReady.Blockers = append(teamReady.Blockers,
				fmt.Sprintf("P95 latency %v exceeds %v threshold",
					team.PerformanceP95, qm.thresholds.MaxPerformanceP95))
		}
		...
		readiness.TeamReadiness[teamName] = teamReady
	}

	// Update recommendation
	if !readiness.OverallReady {
		readiness.Recommendation = "Not ready to merge - quality gates not met"
		blockerCount := 0
		for _, team := range readiness.TeamReadiness {
			blockerCount += len(team.Blockers)
		}
		readiness.BlockerCount = blockerCount
	}

	return readiness, nil
}

// MergeReadiness represents the merge readiness status
type MergeReadiness struct {
	Timestamp      time.Time                `json:"timestamp"`
	OverallReady   bool                     `json:"overall_ready"`
	TeamReadiness  map[string]TeamReadiness `json:"team_readiness"`
	QualityGates   QualityGates             `json:"quality_gates"`
	Recommendation string                   `json:"recommendation"`
	BlockerCount   int                      `json:"blocker_count"`
}

// TeamReadiness represents a team's merge readiness
type TeamReadiness struct {
	TeamName    string        `json:"team_name"`
