...
func (sv *SystemValidator) CheckDockerDaemon(ctx context.Context) error {
	...
	if version == "" {
		return fmt.Errorf("Docker version output is empty")
	}
...
func (sv *SystemValidator) CheckDockerDiskSpace(ctx context.Context) error {
	...
	if err != nil {
		...
		output, err = cmd.Output()
		if err != nil {
			return fmt.Errorf("failed to check disk space for root directory: %v", err)
		}
	}

	lines := strings.Split(string(output), "\n")
	if len(lines) < 2 {
		return fmt.Errorf("unexpected df output: expected at least 2 lines, got %d", len(lines))
	}

	fields := strings.Fields(lines[1])
	if len(fields) < 4 {
		return fmt.Errorf("unexpected df output format: expected at least 4 fields, got %d", len(fields))
	}

	availStr := strings.TrimSuffix(fields[3], "G")
	availGB, err := strconv.Atoi(availStr)
	if err != nil {
		return fmt.Errorf("failed to parse available disk space '%s': %v", availStr, err)
	}

	const minSpaceGB = 5
	if availGB < minSpaceGB {
		return fmt.Errorf("insufficient Docker disk space: %dGB available, minimum %dGB required", availGB, minSpaceGB)
	}
...
func (sv *SystemValidator) CheckDiskSpace(ctx context.Context) error {
	...
	if strings.Contains(outputStr, "100%") || strings.Contains(outputStr, "99%") || strings.Contains(outputStr, "98%") {
		return fmt.Errorf("disk space critically low: usage at 98%% or higher")
	}
...
func (sv *SystemValidator) CheckKubernetesContext(ctx context.Context) error {
	...
	output, err := cmd.Output()
	if err != nil {
		return fmt.Errorf("failed to get current kubectl context: %v", err)
	}

	context := strings.TrimSpace(string(output))
	if context == "" {
		return fmt.Errorf("no kubectl context configured")
	}
