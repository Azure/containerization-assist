...
func (sm *SecurityManager) validateSession(sessionID string) error {
	sessionData, err := sm.sessionManager.GetSessionData(context.Background(), sessionID)
	if err != nil {
		return fmt.Errorf("session validation failed: %w", err)
	}

	// Get session state for expiration check
	sessionState, err := sm.sessionManager.GetSession(context.Background(), sessionID)
	if err != nil {
