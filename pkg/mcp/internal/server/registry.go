...
// Registry and health check functionality consolidated from multiple server files
...
type HealthRecommendation struct {
	...
	Registry    string `json:"registry"`     // Affected registry
	Issue       string `json:"issue"`        // Problem description
	Suggestion  string `json:"suggestion"`   // Recommended action
	Impact      string `json:"impact"`       // Impact if not addressed
...
func CheckRegistryHealth(ctx context.Context, args CheckRegistryHealthArgs) (*CheckRegistryHealthResult, error) {
	...
	// Set default registries if none provided
	...
	for _, registry := range registries {
		...
		if health.IsHealthy {
			...
			// Add recommendation for unhealthy registry
...
func checkSingleRegistry(ctx context.Context, registry string, detailed, forceRefresh bool) (*RegistryHealthInfo, error) {
	...
	// Real implementation would use coredocker package
	...
	if strings.Contains(registry, "docker.io") {
		// Docker Hub is usually reliable
		health.IsHealthy = true
		health.ResponseTime = 100 * time.Millisecond
	} else {
		// Other registries might vary
		health.IsHealthy = true
...
// GetJobStatus functionality (consolidated from get_job_status.go)
...
// GetLogs functionality (consolidated from get_logs.go)
type GetLogsArgs struct {
	...
	// Additional fields expected by gomcp_tools
...
func (t *CheckRegistryHealthTool) GetMetadata() core.ToolMetadata {
...
// GetJobStatusTool implements the job status check tool
...
// GetLogsTool implements the log retrieval tool
type GetLogsTool struct{}

// GetMetadata returns tool metadata
func (t *GetLogsTool) GetMetadata() core.ToolMetadata {
	return core.ToolMetadata{
		Name:        "get_logs",
