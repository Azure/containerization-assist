...
func (t *AtomicAnalyzeRepositoryTool) ExecuteWithContext(serverCtx *server.Context, args AtomicAnalyzeRepositoryArgs) (*AtomicAnalysisResult, error) {
	...
	adapter := NewGoMCPProgressAdapter(serverCtx, interfaces.StandardAnalysisStages())

	// Execute with progress tracking
	ctx := context.Background()
	result, err := t.performAnalysis(ctx, args, adapter)

	// Complete progress tracking
	if err != nil {
		adapter.Complete("Analysis failed")
		if result != nil {
			result.Success = false
		}
		return result, nil // Return result with error info, not the error itself
	} else {
		adapter.Complete("Analysis completed successfully")
	}
...
func (t *AtomicAnalyzeRepositoryTool) performAnalysis(ctx context.Context, args AtomicAnalyzeRepositoryArgs, reporter interfaces.ProgressReporter) (*AtomicAnalysisResult, error) {
	...
	if err != nil {
		...
		result := &AtomicAnalysisResult{
			...
			BaseAIContextResult:        NewBaseAIContextResult("analysis", false, time.Since(startTime)),
	...
	// Stage 1: Initialize
	if reporter != nil {
		reporter.ReportStage(0.0, "Initializing repository analysis")
	}

	// Create base response
	result := &AtomicAnalysisResult{
		BaseToolResponse:           types.NewBaseResponse("atomic_analyze_repository", session.SessionID, args.DryRun),
		BaseAIContextResult:        NewBaseAIContextResult("analysis", false, 0), // Duration and success will be updated later
	...
	if reporter != nil {
		reporter.NextStage("Cloning repository")
	}

	if t.isURL(args.RepoURL) {
		if reporter != nil {
			reporter.ReportStage(0.1, "Starting repository clone")
		}
		...
		if reporter != nil {
			reporter.ReportStage(0.9, "Repository clone completed")
		}
	} else {
		...
		if reporter != nil {
			reporter.ReportStage(0.9, "Using local repository")
		}
	}

	// Stage 3: Analyze repository
	if reporter != nil {
		reporter.NextStage("Analyzing repository structure")
	}
	...
	if session.ScanSummary != nil && session.ScanSummary.RepoPath == result.CloneDir {
		...
		if time.Since(session.ScanSummary.CachedAt) < time.Hour {
			if reporter != nil {
				reporter.ReportStage(0.1, "Using cached analysis results")
			}
			t.logger.Info().
				Str("session_id", session.SessionID).
			...
			if reporter != nil {
				reporter.ReportStage(1.0, "Cached analysis loaded")
			}
	...
	if reporter != nil {
		reporter.ReportStage(0.1, "Starting repository analysis")
	}
	...
	if reporter != nil {
		reporter.ReportStage(0.9, "Repository analysis completed")
	}

	// Stage 4: Generate analysis context
	if reporter != nil {
		reporter.NextStage("Generating analysis context")
	}

	// Analysis context already generated by repository module
	if reporter != nil {
		reporter.ReportStage(0.5, "Analysis context generated")
	}
	...
	if reporter != nil {
		reporter.ReportStage(0.9, "Containerization assessment completed")
	}

	// Stage 5: Finalize and save results
	if reporter != nil {
		reporter.NextStage("Saving analysis results")
	}
	...
	if reporter != nil {
		reporter.ReportStage(0.8, "Session state updated")
	}
	...
	if reporter != nil {
		reporter.ReportStage(1.0, "Analysis complete")
	}
