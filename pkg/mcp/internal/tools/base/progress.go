...
type ProgressCallback func(progress float64, stage string, message string)
...
func (t *ProgressTracker) AddCallback(callback ProgressCallback) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
// AddCallback adds a progress callback
func (r *BatchProgressReporter) AddCallback(callback ProgressCallback) {
	r.mu.Lock()
	defer r.mu.Unlock()
