...
func (v *ImageValidator) validateImage(img ImageInfo, result *ValidationResult) {
	...
	if img.Tag == "" || img.Tag == "latest" {
		result.Warnings = append(result.Warnings, ValidationWarning{
			Type:       "image_tag",
			Line:       img.Line,
			Message:    fmt.Sprintf("Base image '%s' uses 'latest' tag or no tag", img.Image),
	...
	if len(v.trustedRegistries) > 0 && !v.isTrustedRegistry(img.Registry) {
		result.Warnings = append(result.Warnings, ValidationWarning{
			...
			Line:       img.Line,
			Message:    fmt.Sprintf("Base image from untrusted registry: %s", img.Registry),
			Suggestion: "Use images from trusted registries",
			Impact:     "security",
		})
	}
	...
	if deprecated, suggestion := v.isDeprecatedImage(img.Image); deprecated {
		result.Warnings = append(result.Warnings, ValidationWarning{
			...
			Line:       img.Line,
			Message:    fmt.Sprintf("Base image '%s' is deprecated", img.Image),
	...
	for i, img := range images {
		if img.StageName == "" && i < len(images)-1 {
			result.Warnings = append(result.Warnings, ValidationWarning{
				Type:       "unnamed_stage",
				Line:       img.Line,
				Message:    "Intermediate build stage without name",
				Suggestion: "Name build stages with 'AS <name>' for clarity",
	...
	for _, img := range images[:len(images)-1] { // Skip final stage
		if img.StageName != "" && !stageReferences[img.StageName] {
			result.Warnings = append(result.Warnings, ValidationWarning{
				Type:       "unused_stage",
				Line:       img.Line,
				Message:    fmt.Sprintf("Build stage '%s' appears to be unused", img.StageName),
				Suggestion: "Remove unused build stages or reference them with COPY --from",
				Impact:     "build_time",
			})
		}
	}
