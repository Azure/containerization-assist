...
// filterBySeverity filters validation results by minimum severity
func (v *SyntaxValidator) filterBySeverity(result *ValidationResult, minSeverity string) {
	...
	for _, warn := range result.Warnings {
		...
		if warn.Impact == "security" {
			warnSeverity = "error"
		}
		if getSeverityLevel(warnSeverity) >= severityLevel {
...
func (v *SyntaxValidator) checkInstructionCase(lines []string, result *ValidationResult) {
	...
	if upperCount > 0 && lowerCount > 0 {
		result.Warnings = append(result.Warnings, ValidationWarning{
			Type:       "style",
			Line:       0,
			Message:    "Inconsistent instruction casing detected",
			Suggestion: "Use consistent casing for Dockerfile instructions (preferably uppercase)",
	...
	for i, line := range lines {
		...
		if strings.Contains(trimmed, "\\") && !strings.HasSuffix(trimmed, "\\") {
			result.Warnings = append(result.Warnings, ValidationWarning{
				Type:       "syntax",
				Line:       i + 1,
				Message:    "Backslash should be at the end of the line for continuation",
				Suggestion: "Move backslash to the end of the line",
				Impact:     "correctness",
			})
		}
