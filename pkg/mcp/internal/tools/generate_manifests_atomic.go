...
func (t *AtomicGenerateManifestsTool) ExecuteWithFixes(ctx context.Context, args AtomicGenerateManifestsArgs) (*AtomicGenerateManifestsResult, error) {
	...
	if err != nil {
		return &AtomicGenerateManifestsResult{
			...
			BaseAIContextResult: NewBaseAIContextResult("generate_manifests", false, time.Since(time.Now())),
			Success:             false,
...
func (t *AtomicGenerateManifestsTool) performManifestGeneration(ctx context.Context, args AtomicGenerateManifestsArgs, reporter mcptypes.ProgressReporter) (*AtomicGenerateManifestsResult, error) {
	...
	if err != nil {
		result := &AtomicGenerateManifestsResult{
			...
			BaseAIContextResult:       NewBaseAIContextResult("generate_manifests", false, time.Since(startTime)),
			SessionID:                 args.SessionID,
	...
	result := &AtomicGenerateManifestsResult{
		...
		BaseAIContextResult:       NewBaseAIContextResult("generate_manifests", false, 0), // Will update success and duration later
	...
	if templateContext != nil {
		t.logger.Info().
			Str("template", templateContext.SelectedTemplate).
			Str("type", templateContext.TemplateType).
			Str("app_type", templateContext.ApplicationType).
	...
	// Update BaseAIContextResult with final values
