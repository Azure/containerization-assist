...
func (h *SecretsHandler) ScanForSecrets(environment []SecretRef) ([]SecretInfo, error) {
...
func (h *SecretsHandler) GenerateSecretManifests(secrets []SecretInfo, namespace string) ([]GeneratedManifest, error) {
	if len(secrets) == 0 {
		return nil, nil
	}
...
func (h *SecretsHandler) ExternalizeSecrets(environment []SecretRef, secrets []SecretInfo) ([]SecretRef, error) {
	h.logger.Info().
...
func (h *SecretsHandler) generateSecretManifest(secretName string, secrets []SecretInfo, namespace string) (GeneratedManifest, error) {
	// Build secret data
	...
	if err != nil {
		return GeneratedManifest{}, fmt.Errorf("failed to generate secret: %w", err)
	}
