...
func (a *APIKeyScanner) IsApplicable(content string, contentType security.ContentType) bool {
	// API key scanner is applicable to most content types
	return true
}

// Scan performs API key scanning
func (a *APIKeyScanner) Scan(ctx context.Context, config security.ScanConfig) (*security.ScanResult, error) {
	startTime := time.Now()
...
func (a *APIKeyScanner) scanLineForAPIKeys(line string, lineNum int, config security.ScanConfig) ([]security.Secret, error) {
...
func (a *APIKeyScanner) getPatternSeverity(patternName string) security.Severity {
	severityMap := map[string]security.Severity{
		"AWS_Secret_Key":      security.SeverityCritical,
		"Stripe_Secret":       security.SeverityCritical,
		"GitHub_Classic":      security.SeverityHigh,
		"GitHub_Fine_Grained": security.SeverityHigh,
		"Google_API":          security.SeverityHigh,
		"Slack_Token":         security.SeverityHigh,
		"Discord_Bot":         security.SeverityMedium,
		"JWT":                 security.SeverityMedium,
		"Bearer_Token":        security.SeverityMedium,
	}

	if severity, exists := severityMap[patternName]; exists {
		return severity
	}
	return security.SeverityMedium // Default severity
}
...
func (a *APIKeyScanner) getAPIKeySeverity(pattern *APIKeyPattern, confidence float64) security.Severity {
...
func (a *APIKeyScanner) calculateConfidence(result *security.ScanResult) float64 {
