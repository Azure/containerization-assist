...
func (c *CertificateScanner) GetScanTypes() []string {
	return []string{
		string(security.SecretTypePrivateKey),
		string(security.SecretTypeCertificate),
	}
}

// IsApplicable determines if this scanner should run
func (c *CertificateScanner) IsApplicable(content string, contentType security.ContentType) bool {
...
func (c *CertificateScanner) Scan(ctx context.Context, config security.ScanConfig) (*security.ScanResult, error) {
	startTime := time.Now()
...
func (c *CertificateScanner) scanForCertificateBlocks(config security.ScanConfig) ([]security.Secret, error) {
	...
	blockPatterns := map[string]struct {
		...
		"RSA_Private_Key": {
			...
			secretType: security.SecretTypePrivateKey,
		...
		"Certificate": {
			...
			secretType: security.SecretTypeCertificate,
...
func (c *CertificateScanner) scanLineForCertificates(line string, lineNum int, config security.ScanConfig) ([]security.Secret, error) {
	var secrets []security.Secret
...
func (c *CertificateScanner) createCertificateSecret(
	...
	secret := security.Secret{
		...
		Pattern:    patternName,
		Entropy:    security.CalculateEntropy(content),
...
func (c *CertificateScanner) calculateCertificateConfidence(secretType security.SecretType, content, context string) float64 {
	...
	if secretType == security.SecretTypePrivateKey {
		confidence += 0.05
	}
...
func (c *CertificateScanner) calculateConfidence(result *security.ScanResult) float64 {
