...
func (t *AtomicTagImageTool) ExecuteTag(ctx context.Context, args AtomicTagImageArgs) (*AtomicTagImageResult, error) {
	...
	result := &AtomicTagImageResult{
		...
		BaseAIContextResult: NewBaseAIContextResult("tag", false, 0), // Will be updated later
	...
	result.BaseAIContextResult = NewBaseAIContextResult("tag", result.Success, result.TotalDuration)
...
// AI Context is provided by embedded BaseAIContextResult
