...
func (t *AtomicValidateDockerfileTool) performValidation(ctx context.Context, args AtomicValidateDockerfileArgs, reporter mcptypes.ProgressReporter) (*AtomicValidateDockerfileResult, error) {
	...
	if err != nil {
		result := &AtomicValidateDockerfileResult{
			...
			BaseAIContextResult: NewBaseAIContextResult("validate", false, 0), // Will be updated later
...
// GetAIContext is now provided by embedded BaseAIContextResult

// EnrichWithInsights implements *ai_context.ToolContextEnriched
// EnrichWithInsights is now provided by embedded BaseAIContextResult
// GetMetadataForAI is now provided by embedded BaseAIContextResult
