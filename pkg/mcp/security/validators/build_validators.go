...
func (dv *DockerfileValidator) ValidateDockerfile(dockerfileContent string) *core.BuildValidationResult {
	...
	if strings.TrimSpace(dockerfileContent) == "" {
		result.Valid = false
		emptyError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateInstruction(instruction, line string, lineNum int, result *core.BuildValidationResult) {
	switch instruction {
	case "FROM":
	...
	default:
		...
		if !found {
			unknownError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateFromInstruction(line string, lineNum int, result *core.BuildValidationResult) {
...
func (dv *DockerfileValidator) validateCopyAddInstruction(instruction, line string, lineNum int, result *core.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 3 {
		copyError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateExposeInstruction(line string, lineNum int, result *core.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		exposeError := &core.ValidationError{
	...
	for _, port := range parts[1:] {
		if !portRegex.MatchString(port) {
			portError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateUserInstruction(line string, lineNum int, result *core.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		userError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateWorkdirInstruction(line string, lineNum int, result *core.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		workdirError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateCmdEntrypointInstruction(instruction, line string, lineNum int, result *core.BuildValidationResult) {
	parts := strings.Fields(line)
	if len(parts) < 2 {
		cmdError := &core.ValidationError{
...
func (dv *DockerfileValidator) validateStructure(instructions []string, result *core.BuildValidationResult) {
...
func (dv *DockerfileValidator) addGeneralSuggestions(dockerfileContent string, result *core.BuildValidationResult) {
