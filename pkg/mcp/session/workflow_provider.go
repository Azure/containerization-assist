...
func (w *WorkflowLabelProvider) GetK8sLabels(session *SessionState) (map[string]string, error) {
	...
	k8sLabels["mcptypes.session.id"] = session.SessionID

	// Add creation timestamp
	k8sLabels["mcptypes.session.created"] = session.CreatedAt.Format("2006-01-02")
	...
	if session.RepoURL != "" {
		...
		if repoName != "" {
			k8sLabels["mcptypes.repo.name"] = utils.SanitizeForKubernetes(repoName)
		}
	}

	// Add workflow stage if determinable
	if stage := w.determineWorkflowStage(session); stage != "" {
		k8sLabels["mcptypes.workflow.stage"] = stage
	}
