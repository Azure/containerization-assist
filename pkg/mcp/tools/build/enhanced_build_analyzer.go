...
// NewEnhancedBuildAnalyzerFromUnified creates a new enhanced build analyzer from unified Analyzer
func NewEnhancedBuildAnalyzerFromUnified(
	analyzer core.Analyzer,
	logger *slog.Logger,
) *EnhancedBuildAnalyzer {
	// The unified analyzer implements both interfaces
	return NewEnhancedBuildAnalyzer(analyzer, analyzer, logger)
}
...
func NewEnhancedBuildAnalyzer(
	...
	analyzer := &EnhancedBuildAnalyzer{
		...
		logger:             logger.With("component", "enhanced_build_analyzer"),
		capabilities: &AnalyzerCapabilities{
...
func (e *EnhancedBuildAnalyzer) AnalyzeFailure(ctx context.Context, failure *AnalysisRequest) (*AnalysisResult, error) {
	...
	if err != nil {
		e.logger.Error("Failed to generate fix strategies", "error", err)
...
func (e *EnhancedBuildAnalyzer) AnalyzeWithRepository(ctx context.Context, request *RepositoryAnalysisRequest) (*RepositoryAwareAnalysis, error) {
	e.logger.Info("Starting repository-aware analysis",
		"session_id", request.SessionID)

	// Perform basic failure analysis
	baseAnalysis, err := e.AnalyzeFailure(ctx, &request.AnalysisRequest)
	if err != nil {
		return nil, errors.NewError().Message("base analysis failed").Cause(err).WithLocation(
	...
	if err != nil {
		e.logger.Warn("Failed to analyze project insights", "error", err)
	...
	if err != nil {
		e.logger.Warn("Failed to analyze dependencies", "error", err)
...
func (e *EnhancedBuildAnalyzer) shareFailureInsights(ctx context.Context, failure *AnalysisRequest, result *AnalysisResult) {
	...
	if err != nil {
		e.logger.Warn("Failed to share failure insights", "error", err)
	}
}
...
// failurePatterns defines error patterns for categorization
// failurePatterns is deprecated - use BuildFailureService instead
// var failurePatterns = map[string][]string{ ... } // REMOVED: Global state eliminated

// categorizeFailure categorizes the type of failure based on the error
// Deprecated: Use BuildFailureService.CategorizeFailure instead
func (e *EnhancedBuildAnalyzer) categorizeFailure(err error) string {
	// NOTE: This method is deprecated. Use BuildFailureService for failure analysis without global state.
	panic("categorizeFailure is deprecated - use BuildFailureService.CategorizeFailure instead")
}

// matchesAnyPattern checks if the error string matches any of the given patterns
func (e *EnhancedBuildAnalyzer) matchesAnyPattern(errStr string, patterns []string) bool {
...
func (e *EnhancedBuildAnalyzer) generateFixStrategies(ctx context.Context, failure *AnalysisRequest, failureType string, relatedFailures []*RelatedFailure) ([]*FixStrategy, error) {
	...
	if e.aiAnalyzer != nil {
		...
		if err != nil {
			e.logger.Warn("Failed to generate AI strategies, falling back to rule-based", "error", err)
		} else {
...
Format as structured text.`,
	...
	if err != nil {
		return nil, errors.NewError().Message("AI analysis failed").Cause(err).WithLocation(
