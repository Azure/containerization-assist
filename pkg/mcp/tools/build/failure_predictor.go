...
func NewFailurePredictor(logger *slog.Logger) *FailurePredictor {
	return &FailurePredictor{
		...
		logger:          logger.With("component", "failure_predictor"),
	}
}
...
func (fp *FailurePredictor) PredictFailures(ctx context.Context, buildContext *AnalysisBuildContext) (*FailurePrediction, error) {
	fp.logger.Info("Starting failure prediction analysis",
		"session_id", buildContext.SessionID,
		"project_type", buildContext.ProjectInfo.Language)
	// Analyze historical patterns
	patterns, err := fp.patternAnalyzer.AnalyzeHistoricalPatterns(buildContext.BuildHistory)
	if err != nil {
		fp.logger.Warn("Failed to analyze historical patterns", "error", err)
	...
	fp.logger.Info("Failure prediction completed",
		"risk_score", finalRiskScore,
		"confidence", confidence,
		"potential_failures", len(potentialFailures))
...
func NewPatternAnalyzer(logger *slog.Logger) *PatternAnalyzer {
...
func NewRiskCalculator(logger *slog.Logger) *RiskCalculator {
	return &RiskCalculator{
		logger: logger.With("component", "risk_calculator"),
	}
}
