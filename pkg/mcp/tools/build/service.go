...
// ValidateSessionID validates a session ID using tag-based validation
func (s *ValidationService) ValidateSessionID(ctx context.Context, sessionID string) error {
	// Create a temporary struct to use tag-based validation
	tempArgs := struct {
	...
	return validation.ValidateTaggedStruct(tempArgs)
}
