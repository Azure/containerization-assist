...
func (t *AtomicGenerateManifestsTool) detectDatabases(ctx *server.Context, session *core.SessionState, workspaceDir string) (*analyze.DatabaseDetectionResult, error) {
	// Check if repository exists in workspace
	repoPath := workspaceDir + "/repo"

	// Create detection parameters
	params := analyze.DatabaseDetectionParams{
...
func (t *AtomicGenerateManifestsTool) addDatabaseEnvironmentVars(args *GenerateManifestsArgs, db analyze.DetectedDatabase) {
	// Add environment variables based on database type
	switch db.Type {
	case analyze.PostgreSQL:
		t.addPostgreSQLEnvVars(args, db)
	case analyze.MySQL:
		t.addMySQLEnvVars(args, db)
	case analyze.MongoDB:
...
func (t *AtomicGenerateManifestsTool) addPostgreSQLEnvVars(args *GenerateManifestsArgs, db analyze.DetectedDatabase) {
...
func (t *AtomicGenerateManifestsTool) addMySQLEnvVars(args *GenerateManifestsArgs, db analyze.DetectedDatabase) {
...
func (t *AtomicGenerateManifestsTool) addMongoDBEnvVars(args *GenerateManifestsArgs, db analyze.DetectedDatabase) {
...
func (t *AtomicGenerateManifestsTool) addRedisEnvVars(args *GenerateManifestsArgs, db analyze.DetectedDatabase) {
