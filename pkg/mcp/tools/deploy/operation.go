...
func (op *Operation) ExecuteOnce(ctx context.Context) error {
	if op.ExecuteFunc == nil {
		return errors.NewError().Messagef("no execute function configured for operation %s", op.Name).Build()
	}
...
func (op *Operation) GetFailureAnalysis(ctx context.Context, err error) (*mcptypes.ConsolidatedFailureAnalysis, error) {
	if err == nil {
		return nil, nil
	}
	...
	analysis := &mcptypes.ConsolidatedFailureAnalysis{
		...
		ConsolidatedErrorContext: fmt.Sprintf("operation=%s, type=%s", op.Name, op.Type),
	}
...
func (op *Operation) PrepareForRetry(ctx context.Context, fixAttempt interface{}) error {
	...
	op.Logger.Debug("No retry preparation needed for operation")
	return nil
}
...
func analyzeDeploymentError(err error) (error, error) {
	...
	return errors.NewError().Messagef("deployment failed: %s (%s)", err.Error(), contextInfo).WithLocation(
...
func analyzeHealthCheckError(err error) (error, error) {
	...
	return errors.NewError().Messagef("health check operation failed: %v", err).WithLocation().Build(), nil
}
