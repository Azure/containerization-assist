...
func NewSecretsHandler(logger *slog.Logger) *SecretsHandler {
	return &SecretsHandler{
		secretScanner: security.NewSecretScanner(),
		// TODO: Update kubernetes.NewSecretGenerator to accept *slog.Logger
		// For now, create a zerolog logger wrapper
		secretGenerator: kubernetes.NewSecretGenerator(createZerologWrapper()),
		logger:          logger.With("component", "secrets_handler"),
	}
}
...
func (h *SecretsHandler) ScanForSecrets(environment []SecretValue) ([]SecretInfo, error) {
	h.logger.Info("Scanning for secrets in environment variables", "env_count", len(environment))
	...
	for _, env := range environment {
		...
		if h.isPotentialSecret(env.Name, env.Value) {
			...
			h.logger.Info("Detected potential secret",
				"name", env.Name,
	...
	h.logger.Info("Secret scanning completed", "secrets_found", len(secrets))
	return secrets, nil
}
...
func (h *SecretsHandler) GenerateSecretManifests(secrets []SecretInfo, namespace string) ([]ManifestFile, error) {
	...
	h.logger.Info("Generating secret manifests",
	...
	for secretName, secretInfos := range secretGroups {
		...
		if err != nil {
			return nil, errors.NewError().Message(fmt.Sprintf("failed to generate secret %s", secretName)).Cause(err).Build()
		}
...
func (h *SecretsHandler) ExternalizeSecrets(environment []SecretValue, secrets []SecretInfo) ([]SecretValue, error) {
	h.logger.Info("Externalizing secrets",
...
func (h *SecretsHandler) generateSecretManifest(secretName string, secrets []SecretInfo, namespace string) (ManifestFile, error) {
	...
	if err != nil {
		return ManifestFile{}, errors.NewError().Message("failed to generate secret").Cause(err).WithLocation(
...
// createZerologWrapper creates a temporary zerolog wrapper
// TODO: Remove this once kubernetes package is migrated to slog
func createZerologWrapper() zerolog.Logger {
