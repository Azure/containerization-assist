...
func NewAPIKeyScanner(logger *slog.Logger) *APIKeyScanner {
	scanner := &APIKeyScanner{
		name:     "api_key_scanner",
		patterns: make(map[string]*APIKeyPattern),
		logger:   logger.With("scanner", "api_key"),
	}
	...
	for patternName, pattern := range a.patterns {
		...
		for _, match := range matches {
			...
			if len(match) >= 2 && match[1] != "" {
				// Pattern has a capture group, use it
				value = match[1]
			} else if len(match) >= 1 {
				// Pattern doesn't have a capture group, use the full match
				value = match[0]
			} else {
...
func (a *APIKeyScanner) initializePatterns() {
	...
	for name, patternStr := range patterns {
		...
		if err != nil {
			a.logger.Error("Failed to compile API key pattern", "error", err, "pattern", name)
			continue
		}
	...
	a.logger.Debug("Initialized API key patterns", "patterns", len(a.patterns))
}
...
func (a *APIKeyScanner) isValidAPIKey(value, patternName string) bool {
	...
	case "GitHub_Classic":
		return len(value) == 44 && strings.HasPrefix(value, "ghp_")
	case "JWT":
