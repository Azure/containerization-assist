...
func NewCertificateScanner(logger *slog.Logger) *CertificateScanner {
	scanner := &CertificateScanner{
		name:     "certificate_scanner",
		patterns: make(map[string]*CertificatePattern),
		logger:   logger.With("scanner", "certificate"),
	}
...
func (c *CertificateScanner) initializePatterns() {
	...
	for name, patternInfo := range patterns {
		...
		if err != nil {
			c.logger.Error("Failed to compile certificate pattern", "error", err, "pattern", name)
			continue
		}
	...
	c.logger.Debug("Initialized certificate patterns", "patterns", len(c.patterns))
}
