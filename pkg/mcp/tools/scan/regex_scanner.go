...
type RegexBasedScanner struct {
	...
	patternOrder []SecretType // Order to check patterns
	logger       *slog.Logger
}

// NewRegexBasedScanner creates a new regex-based scanner
func NewRegexBasedScanner(logger *slog.Logger) *RegexBasedScanner {
	scanner := &RegexBasedScanner{
		name:     "regex_scanner",
		patterns: make(map[SecretType]*regexp.Regexp),
		logger:   logger.With("scanner", "regex"),
	}

	scanner.initializePatterns()

	// Define pattern checking order - more specific patterns first
	scanner.patternOrder = []SecretType{
		SecretTypeEnvironmentVar, // Check environment variables first
		SecretTypeAPIKey,
		SecretTypeToken,
		SecretTypePassword,
		SecretTypeSecret,
...
func (r *RegexBasedScanner) scanLine(line string, lineNum int, config ScanConfig) ([]Secret, error) {
	var secrets []Secret
	foundValues := make(map[string]bool) // Track found values to avoid duplicates

	// Check patterns in defined order
	for _, secretType := range r.patternOrder {
		pattern, exists := r.patterns[secretType]
		if !exists {
			continue
		}
		...
		for _, match := range matches {
			if len(match) > 1 {
				...
				if len(value) > 0 && !foundValues[value] {
...
func (r *RegexBasedScanner) detectHighEntropy(line string, lineNum int, config ScanConfig) []Secret {
	...
	for _, token := range tokens {
		if len(token) >= 16 && len(token) <= 100 { // Reasonable secret length
			...
			if entropy > 0.6 { // High entropy threshold (normalized entropy)
	...
	for _, pattern := range tokenPatterns {
		...
		for _, match := range matches {
			if len(match) > 1 && match[1] != "" {
				tokenMap[match[1]] = true
			} else if len(match) > 0 && match[0] != "" {
				tokenMap[match[0]] = true
			}
		}
	}
...
func (r *RegexBasedScanner) initializePatterns() {
	...
	for secretType, patternStr := range patterns {
		...
		if err != nil {
			r.logger.Error("Failed to compile regex pattern", "error", err, "pattern", patternStr)
			continue
		}
		r.patterns[secretType] = compiled
	}

	r.logger.Debug("Initialized regex patterns", "patterns", len(r.patterns))
}
...
func (r *RegexBasedScanner) calculateEntropyConfidence(entropy float64, value string) float64 {
	// Base confidence from normalized entropy (0-1 range)
	// Entropy > 0.6 is considered high
	confidence := (entropy - 0.5) * 2.0 // Scale from 0.5-1.0 to 0.0-1.0

	// Adjust based on length
	if len(value) >= 20 && len(value) <= 64 {
		confidence += 0.1 // Good length for a secret
	}
	if len(value) < 16 {
		confidence -= 0.2 // Too short
	}
	if len(value) > 100 {
		confidence -= 0.2 // Too long
	}
