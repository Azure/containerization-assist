...
func ValidatePath(path, fieldName string) *core.ValidationError {
	if path == "" {
		return core.NewFieldValidationError(fieldName, "path cannot be empty")
	}
	...
	if strings.Contains(cleanPath, "..") {
		return core.NewFieldValidationError(fieldName, "path traversal not allowed").
...
func ValidateAbsolutePath(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}

	if !filepath.IsAbs(path) {
		return core.NewFieldValidationError(fieldName, "must be an absolute path").
...
func ValidateRelativePath(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}

	if filepath.IsAbs(path) {
		return core.NewFieldValidationError(fieldName, "must be a relative path").
...
func ValidateFileExists(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}
	...
	if os.IsNotExist(err) {
		return core.NewFieldValidationError(fieldName, "file does not exist: "+path)
	}
	if err != nil {
		return core.NewFieldValidationError(fieldName, "cannot access file: "+err.Error())
	}

	if info.IsDir() {
		return core.NewFieldValidationError(fieldName, "path is a directory, not a file: "+path)
	}
...
func ValidateDirectoryExists(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}
	...
	info, err := os.Stat(path)
	if os.IsNotExist(err) {
		return core.NewFieldValidationError(fieldName, "directory does not exist: "+path)
	}
	if err != nil {
		return core.NewFieldValidationError(fieldName, "cannot access directory: "+err.Error())
	}

	if !info.IsDir() {
		return core.NewFieldValidationError(fieldName, "path is a file, not a directory: "+path)
	}
...
func ValidatePathExists(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}
	...
	_, err := os.Stat(path)
	if os.IsNotExist(err) {
		return core.NewFieldValidationError(fieldName, "path does not exist: "+path)
	}
	if err != nil {
		return core.NewFieldValidationError(fieldName, "cannot access path: "+err.Error())
	}
...
func ValidateFileReadable(path, fieldName string) *core.ValidationError {
	...
	if err != nil {
		return core.NewFieldValidationError(fieldName, "file is not readable: "+err.Error())
	}
	file.Close()
...
func ValidateFileWritable(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}
	...
	if _, err := os.Stat(path); err == nil {
		...
		if err != nil {
			return core.NewFieldValidationError(fieldName, "file is not writable: "+err.Error())
		}
		file.Close()
		...
		if err := ValidateDirectoryWritable(dir, fieldName); err != nil {
			return core.NewFieldValidationError(fieldName, "cannot create file in directory: "+err.Error())
		}
	} else {
		return core.NewFieldValidationError(fieldName, "cannot access file: "+err.Error())
	}
...
func ValidateDirectoryWritable(path, fieldName string) *core.ValidationError {
	...
	if err != nil {
		return core.NewFieldValidationError(fieldName, "directory is not writable: "+err.Error())
	}
...
func ValidateFileExtension(path, fieldName string, allowedExtensions []string) *core.ValidationError {
	if path == "" {
		return nil
	}
...
func ValidateDockerfilePath(path, fieldName string) *core.ValidationError {
...
func ValidateKubernetesManifestPath(path, fieldName string) *core.ValidationError {
	if err := ValidatePath(path, fieldName); err != nil {
		return err
	}
...
func ValidateConfigPath(path, fieldName string) *core.ValidationError {
	if err := ValidatePath(path, fieldName); err != nil {
		return err
	}
...
func ValidateWorkingDirectory(path, fieldName string) *core.ValidationError {
	if err := ValidatePath(path, fieldName); err != nil {
		return err
	}
...
func ValidateNoHiddenPaths(path, fieldName string) *core.ValidationError {
	if path == "" {
		return nil
	}
...
func ValidatePathLength(path, fieldName string, maxLength int) *core.ValidationError {
	if path == "" {
		return nil
	}
...
func ValidateSecurePath(path, fieldName string) *core.ValidationError {
	if err := ValidatePath(path, fieldName); err != nil {
		return err
	}
	...
	if strings.Contains(cleanPath, "\x00") {
		return core.NewFieldValidationError(fieldName, "path contains null bytes")
	}
