...
func (i *ImageValidator) Validate(ctx context.Context, data interface{}, options *core.ValidationOptions) *core.ValidationResult {
...
func (i *ImageValidator) validateImageReference(data interface{}, result *core.ValidationResult, options *core.ValidationOptions) {
	...
	if !i.isValidImageFormat(imageRef) {
		result.AddError(core.NewValidationError(
			...
			core.ErrTypeFormat,
			core.SeverityHigh,
		).WithSuggestion("Use format: [registry/]namespace/image[:tag][@digest]"))
		return
	}
	...
	if image.Tag == "latest" || image.Tag == "" {
		...
		warning.ValidationError.WithSuggestion("Use specific version tags for reproducibility")
...
func (i *ImageValidator) validateImageList(data interface{}, result *core.ValidationResult, options *core.ValidationOptions) {
...
func (i *ImageValidator) validateBaseImage(data interface{}, result *core.ValidationResult, options *core.ValidationOptions) {
	...
	if strings.Contains(image.Name, "alpine") {
		...
		warning.ValidationError.WithSuggestion("Test thoroughly or consider using debian-slim if you encounter issues")
		result.AddWarning(warning)
	}
	...
	if image.Name == "scratch" {
		...
		warning.ValidationError.WithSuggestion("Ensure your application is statically compiled")
		result.AddWarning(warning)
	}
...
func (i *ImageValidator) autoDetectAndValidate(data interface{}, result *core.ValidationResult, options *core.ValidationOptions) {
	switch v := data.(type) {
	case string:
		i.validateImageReference(data, result, options)
	case []string:
...
func (i *ImageValidator) checkDeprecatedImage(image *ParsedImage, result *core.ValidationResult) {
...
func (i *ImageValidator) validateRegistryTrust(image *ParsedImage, result *core.ValidationResult, options *core.ValidationOptions) {
	...
	if !trusted {
		...
		warning.ValidationError.WithSuggestion("Consider using images from trusted registries or add this registry to trusted list")
		result.AddWarning(warning)
	}
...
func (i *ImageValidator) checkKnownVulnerabilities(image *ParsedImage, result *core.ValidationResult) {
	...
	for pattern, message := range vulnerableImages {
		if strings.HasPrefix(imageWithTag, pattern) {
			result.AddError(core.NewValidationError(
				...
				core.SeverityHigh,
			).WithSuggestion("Update to a newer, supported version"))
			break
		}
	}
}

func (i *ImageValidator) validateDigest(digest string, result *core.ValidationResult) {
...
func (i *ImageValidator) validatePlatform(platform string, result *core.ValidationResult) {
	...
	if !platformRegex.MatchString(platform) {
		result.AddError(core.NewValidationError(
			...
			core.ErrTypeFormat,
			core.SeverityMedium,
		).WithSuggestion("Use format: os/arch[/variant] (e.g., linux/amd64, linux/arm64/v8)"))
		return
	}
