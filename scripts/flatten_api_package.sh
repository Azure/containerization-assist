...
# Flatten application/api package to top-level api package
# This script moves files and updates all imports

set -e

echo "=== Flattening application/api package ==="

# Base directories
OLD_PKG="pkg/mcp/application/api"
NEW_PKG="pkg/mcp/api"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/application/api"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/api"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package api" to "package api"
    # (no change needed as package name stays the same)
    sed -i 's/^package api$/package api/' "$file"
done
