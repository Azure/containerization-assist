...
# Flatten domain/config package to top-level config package
# This script moves files and updates all imports

set -e

echo "=== Flattening domain/config package ==="

# Base directories
OLD_PKG="pkg/mcp/domain/config"
NEW_PKG="pkg/mcp/config"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/domain/config"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/config"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package config" to "package config"
    # (no change needed as package name stays the same)
    sed -i 's/^package config$/package config/' "$file"
done
