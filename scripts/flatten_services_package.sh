...
# Flatten application/services package to top-level services package
# This script moves files and updates all imports

set -e

echo "=== Flattening application/services package ==="

# Base directories
OLD_PKG="pkg/mcp/application/services"
NEW_PKG="pkg/mcp/services"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/application/services"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/services"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package services" to "package services"
    # (no change needed as package name stays the same)
    sed -i 's/^package services$/package services/' "$file"
done
