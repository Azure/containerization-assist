...
# Flatten domain/session package to top-level session package
# This script moves files and updates all imports

set -e

echo "=== Flattening domain/session package ==="

# Base directories
OLD_PKG="pkg/mcp/domain/session"
NEW_PKG="pkg/mcp/session"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/domain/session"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/session"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package session" to "package session"
    # (no change needed as package name stays the same)
    sed -i 's/^package session$/package session/' "$file"
done
...
echo ""
echo "Next steps:"
echo "1. Run 'make mcp' to test compilation"
