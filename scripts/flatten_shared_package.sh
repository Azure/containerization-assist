...
# Flatten domain/shared package to top-level shared package
# This script moves files and updates all imports

set -e

echo "=== Flattening domain/shared package ==="

# Base directories
OLD_PKG="pkg/mcp/domain/shared"
NEW_PKG="pkg/mcp/shared"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/domain/shared"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/shared"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package shared" to "package shared"
    # (no change needed as package name stays the same)
    sed -i 's/^package shared$/package shared/' "$file"
done
