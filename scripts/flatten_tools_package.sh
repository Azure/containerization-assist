...
# Flatten domain/tools package to top-level tools package
# This script moves files and updates all imports

set -e

echo "=== Flattening domain/tools package ==="

# Base directories
OLD_PKG="pkg/mcp/domain/tools"
NEW_PKG="pkg/mcp/tools"
OLD_IMPORT="github.com/Azure/container-kit/pkg/mcp/domain/tools"
NEW_IMPORT="github.com/Azure/container-kit/pkg/mcp/tools"
...
find "$NEW_PKG" -name "*.go" -type f | while read -r file; do
    # Update package declaration from "package tools" to "package tools"
    # (no change needed as package name stays the same)
    sed -i 's/^package tools$/package tools/' "$file"
done
