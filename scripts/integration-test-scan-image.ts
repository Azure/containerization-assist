 ...
 * - Advisory mode for live baseline images that may accumulate CVEs over time
 *
 * Usage:
 *   npm run build
...
interface TestCase {
  ...
  /**
   * When true, threshold and severity mismatches emit warnings instead of
   * failing the test. Use for live images whose CVE counts drift over time.
   */
  advisory?: boolean;
...
interface TestResult {
  name: string;
  passed: boolean;
  /** True when the test had issues but was advisory-only */
  advisoryWarning?: boolean;
  message: string;
  vulnerabilities?: {
    critical: number;
    high: number;
...
const TEST_CASES: TestCase[] = [
  ...
  },
  {
    ...
    advisory: true, // Live images accumulate CVEs; warn instead of failing the suite
    scanner: 'trivy',
...
async function main() {
  ...
  for (const testCase of TEST_CASES) {
    ...
    if (testCase.advisory) {
      console.log(`      Mode: Advisory (mismatches warn, do not fail)`);
    }
    ...
    try {
      ...
      // Advisory tests: warn on mismatch instead of failing the suite
      if (!validation.passed && testCase.advisory) {
        console.log(`      ⚠️  ADVISORY WARNING (not a failure)`);
        for (const msg of validation.messages) {
          console.log(`         - ${msg}`);
        }
        console.log(`      ℹ️  Live image CVE drift detected — this is expected over time.`);
        results.push({
          name: testCase.name,
          passed: true,
          advisoryWarning: true,
          message: `Advisory: ${validation.messages.join('; ')}`,
          vulnerabilities: {
            critical: vulns.critical,
            high: vulns.high,
            medium: vulns.medium,
  ...
  if (advisoryWarnCount > 0) {
    console.log(`   Advisory warnings: ${advisoryWarnCount} ⚠️`);
  }
  console.log('\n   Results by test case:');

  for (const result of results) {
    const status = result.passed ? (result.advisoryWarning ? '⚠️ WARN' : '✅ PASS') : '❌ FAIL';
    const duration = result.duration ? ` (${(result.duration / 1000).toFixed(1)}s)` : '';
    console.log(`   ${status} ${result.name}${duration}`);
    if (!result.passed || result.advisoryWarning) {
  ...
  const resultsJson = {
    ...
    passed: passCount,
    failed: failCount,
    advisoryWarnings: advisoryWarnCount,
    ...
    results: results.map((r) => ({
      name: r.name,
      passed: r.passed,
      advisoryWarning: r.advisoryWarning ?? false,
      message: r.message,
  ...
  if (advisoryWarnCount > 0) {
    console.log(
      `✅ All tests passed! (${advisoryWarnCount} advisory warning(s) — live image CVE drift)`,
    );
  } else {
