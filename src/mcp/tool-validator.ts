/**
 * AI-Powered Parameter Validation (Functional)
 *
...
export const validateParameters = async (
  deps: AIValidatorDeps,
  toolName: string,
  parameters: Record<string, unknown>,
  context?: ValidationContext,
): Promise<Result<ValidationResult>> => {
  const startTime = Date.now();
  const logger = deps.logger.child({ component: 'AIParameterValidator' });
  logger.info({ toolName }, 'AI parameter validation requested');
  ...
  try {
    ...
    if (isAIValidationAvailable(deps)) {
      const aiValidation = await performAIValidation(deps, toolName, parameters, context);

      if (aiValidation.ok) {
        const combined = combineValidationResults(basicValidation, aiValidation.value);
...
export const suggestParameters = async (
  ...
  const logger = deps.logger.child({ component: 'AIParameterValidator' });
  logger.info({ toolName }, 'AI parameter suggestions requested');
  ...
  try {
    ...
    logger.error({ toolName, error: message }, 'Parameter suggestion failed');
 ...
 */
export const isAIValidationAvailable = (deps: AIValidatorDeps): boolean => {
  return !!deps.promptRegistry;
};

/**
 * Get validation capabilities
 */
export const getValidationCapabilities = (
  deps: AIValidatorDeps,
): {
  ...
  return {
    aiValidation: isAIValidationAvailable(deps),
    contextAware: true,
    suggestions: true,
    supportedTools: [
      'analyze-repo',
...
const performAIValidation = async (
  ...
  try {
    ...
    if (deps.promptRegistry?.getPrompt) {
      await deps.promptRegistry.getPrompt('parameter-validation', {
        toolName,
        parameters: JSON.stringify(parameters, null, 2),
        context: context ? JSON.stringify(context, null, 2) : undefined,
        validationRules: getValidationRules(toolName),
      });
    }
 ...
 */
const getValidationRules = (toolName: string): string => {
...
const combineValidationResults = (
  ...
  const combinedWarnings = [
    ...new Set([...(basicResult.warnings || []), ...(aiResult.warnings || [])]),
  ];
  ...
  return {
    ...
    confidence: Math.max(basicResult.confidence || 0, aiResult.confidence || 0),
    metadata: {
      validationTime: basicResult.metadata?.validationTime || 0,
      aiEnhanced: true,
      rulesApplied: [
        ...(basicResult.metadata?.rulesApplied || []),
        ...(aiResult.metadata?.rulesApplied || []),
      ],
    },
  };
};
...
const handleValidationError = (
  ...
  const message = extractErrorMessage(error);
  logger.error({ error: message }, 'Parameter validation error');
...
export const createAIParameterValidator = (deps: AIValidatorDeps) => ({
  validateParameters: (
    ...
    parameters: Record<string, unknown>,
    context?: ValidationContext,
  ) => validateParameters(deps, toolName, parameters, context),
  suggestParameters: (
    toolName: string,
    partialParameters: Record<string, unknown>,
    context?: ValidationContext,
  ) => suggestParameters(deps, toolName, partialParameters, context),
  isAIValidationAvailable: () => isAIValidationAvailable(deps),
  getCapabilities: () => getValidationCapabilities(deps),
});
