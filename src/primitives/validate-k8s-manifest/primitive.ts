...
import type { ValidateK8sManifestInput } from './schema';
import { toFinding, type ValidatePolicyOut } from '../types';

async function handleValidateK8sManifest(
  input: ValidateK8sManifestInput,
  ctx: ToolContext,
  ...
  try {
    ...
    return Failure(`validate-k8s-manifest failed while evaluating policy: ${cause}`, {
      message: cause,
...
export default tool({
  ...
  handler: handleValidateK8sManifest,
});
