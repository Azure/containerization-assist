...
import {
  fixDockerfilePlanSchema,
  type DockerfileFixPlan,
  type FixDockerfilePlanParams,
...
const name = 'fix-dockerfile-plan';
...
interface ExtendedInput extends FixDockerfilePlanParams {
 ...
 */
async function run(
  input: z.infer<typeof fixDockerfilePlanSchema>,
  ctx: ToolContext,
...
const tool: MCPTool<typeof fixDockerfilePlanSchema, DockerfileFixPlan> = {
  name,
  description,
  category: 'docker',
  version,
  schema: fixDockerfilePlanSchema,
  metadata: {
    knowledgeEnhanced: true,
    samplingStrategy: 'none',
