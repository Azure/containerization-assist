...
async function handlePrepareCluster(
  ...
  try {
    ...
    // createKubernetesClient throws synchronously when no kubeconfig exists (e.g.
    ...
    try {
      ...
      logger.debug(
        { error },
        'Kubeconfig not available — cluster may not exist yet; assuming namespace does not exist',
      );
    }
