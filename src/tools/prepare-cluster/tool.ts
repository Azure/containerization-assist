...
async function handlePrepareCluster(
  ...
  try {
    ...
    // Guard the Kubernetes client creation: in a fresh environment (no cluster yet),
    // the kubeconfig may not exist. The tool is advisory, so no kubeconfig is fine —
    // we just assume the namespace does not exist and skip the namespace probe.
    ...
    try {
      ...
      logger.debug(
        { error: String(error) },
        'Kubeconfig not available; namespace existence check skipped',
      );
    }
