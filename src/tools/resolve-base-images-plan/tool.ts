...
import {
  resolveBaseImagesPlanSchema,
  type BaseImagePlan,
  type BaseImageRecommendation,
  type BaseImageRequirement,
  type ResolveBaseImagesPlanParams,
} from './schema';
...
const name = 'resolve-base-images-plan';
const description =
...
async function run(
  input: z.infer<typeof resolveBaseImagesPlanSchema>,
  ctx: ToolContext,
...
const tool: MCPTool<typeof resolveBaseImagesPlanSchema, BaseImagePlan> = {
  name,
  description,
  category: 'docker',
  version,
  schema: resolveBaseImagesPlanSchema,
  metadata: {
    knowledgeEnhanced: true,
    samplingStrategy: 'none',
