...
const DEFAULT_TIMEOUTS = {
  healthCheckMs: 5000, // Poll interval for health checks in milliseconds
};
 ...
 * @param timeoutSeconds - Timeout in seconds for health check
 */
async function checkDeploymentHealth(
  ...
  const pollIntervalMs = DEFAULT_TIMEOUTS.healthCheckMs || 5000;
...
async function checkEndpointHealth(url: string): Promise<boolean> {
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), DEFAULT_TIMEOUTS.healthCheckMs || 5000);
...
async function verifyDeploymentImpl(
  ...
  const parsed = verifyDeploymentSchema.safeParse(params);
  if (!parsed.success) {
    return Failure(`Invalid parameters: ${parsed.error.message}`);
  }
  ...
  try {
    ...
    const timeoutSeconds = 60; // Timeout in seconds for deployment verification
    ...
    const sessionResult = await ensureSession(context, validParams.sessionId);
    if (!sessionResult.ok) {
      return Failure(sessionResult.error);
    }

    const { id: sessionId, state: session } = sessionResult.value;
    const slice = useSessionSlice('verify-deployment', context);
    ...
    await slice.patch(sessionId, { input: validParams });

    const k8sClient = createKubernetesClient(logger);

    // Get deployment info from session or config
    const sessionData = session as SessionData;
    const deploymentResult = sessionData?.deploymentResult as
      | {
          namespace?: string;
          deploymentName?: string;
          serviceName?: string;
...
export const verifyDeploy = {
  ...
  name: 'verify-deploy',
  description: 'Verify deployment status in Kubernetes cluster',
  inputSchema: verifyDeploymentSchema,
