...
describe('Package Integrity', () => {
  ...
  describe('npm install', () => {
    ...
    it('should install package successfully', () => {
      const nodeModulesPath = join(installDir, 'node_modules/containerization-assist-mcp-mcp');
      expect(existsSync(nodeModulesPath)).toBe(true);
    });

    it('should have correct package.json structure', () => {
      const packageJsonPath = join(installDir, 'node_modules/containerization-assist-mcp-mcp/package.json');
      const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf8'));

      expect(packageJson.name).toBe('containerization-assist-mcp-mcp');
  ...
  describe('Programmatic Import', () => {
    ...
    it('should support main import', async () => {
      const testScript = `
        import pkg from 'containerization-assist-mcp-mcp';
    ...
    it('should support named exports', async () => {
      const testScript = `
        import { MCPServer } from 'containerization-assist-mcp-mcp';
