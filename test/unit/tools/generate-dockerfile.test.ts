  ...
  describe('Happy Path', () => {
    it('should generate Dockerfile plan for Node.js project', async () => {
      ...
      if (result.ok) {
        ...
        expect(result.value.summary).toContain('Dockerfile Planning Summary');
      }
    });
...
CMD ["node", "index.js"]`;
      ...
      if (result.ok) {
        ...
        expect(result.value.summary).toContain('ENHANCE existing Dockerfile');
      }
    });
...
CMD ["node", "index.js"]`;
    ...
    it('should handle Dockerfile read errors gracefully', async () => {
      ...
      if (result.ok) {
        ...
        expect(result.value.summary).toContain('CREATE new Dockerfile');
      }
    });
  });
