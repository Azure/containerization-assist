...
var generatedToolRegistry = map[string]func() Tool{
{{range .Tools}}	"{{.ToolName}}": func() Tool { return &{{.Name}}{} },
{{end}}
}
...
func RegisterAllTools(registry ToolRegistry) error {
	for name, factory := range generatedToolRegistry {
		tool := factory()
		if err := registry.RegisterTool(name, tool); err != nil {
...
func discoverTools() ([]ToolInfo, error) {
	...
	searchPaths := []string{
		...
		"pkg/mcp/internal/build",     // Future location after restructuring
		"pkg/mcp/internal/deploy",    // Future location after restructuring
		"pkg/mcp/internal/scan",      // Future location after restructuring
		"pkg/mcp/internal/analyze",   // Future location after restructuring
	}
...
func discoverToolsInFile(filePath string) ([]ToolInfo, error) {
	...
	packagePath := strings.TrimSuffix(filepath.Dir(filePath), "/")
